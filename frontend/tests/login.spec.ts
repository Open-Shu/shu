...
import { mockGoogleLibrary, mockPublicConfigRequest, mockGoogleAuthRequest, mockAuthMeRequest, blockGoogleIdentityScript } from './mocks'
...
test.describe('Login page', () => {
  ...
  test('Website opens and redirects', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Sign-in to Max' })).toBeVisible();
  });
  ...
  test('Registration - Success', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Full Name' }).fill('test');
    await page.getByRole('textbox', { name: 'Email Address' }).fill(TEST_EMAIL);
    await page.getByRole('textbox', { name: 'Password', exact: true }).fill('testtest');
    await page.getByRole('textbox', { name: 'Confirm Password' }).fill('testtest');
    await page.getByRole('button', { name: 'Create Account' }).click();
    await expect(page.getByRole('heading')).toContainText('Registration Successful!');
  });
  ...
  test('Registration - Email already exists', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Confirm Password' }).fill('testtest');
    await page.getByRole('button', { name: 'Create Account' }).click();
    await expect(page.getByRole('alert')).toContainText(`User with email ${TEST_EMAIL} already exists`);
  });
  ...
  test('Login - Inactive account', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Password' }).fill('test');
    await page.getByRole('button', { name: 'Sign In', exact: true }).click();
    await expect(page.getByRole('alert')).toContainText('User account is inactive. Please contact an administrator for activation.');
  });
  ...
  test('Login - Google sign in succeeds', async ({ page }) => {
    ...
    await expect(page.getByRole('paragraph')).toContainText('Sign in with your Google account to continue');
    await page.getByRole('button', { name: 'Sign in with Google' }).click();
    ...
    await expect.poll(async () => {
      return await page.evaluate(() => localStorage.getItem('shu_refresh_token'));
    }).toBe('playwright-refresh-token');

    await waitForLoginComplete(page);
  });

  test('Login - Using password with existing Google account', async ({ page }) => {
    await db.removeUser(TEST_EMAIL);
    ...
    await page.getByRole('textbox', { name: 'Password' }).fill('testtest');
    await page.getByRole('button', { name: 'Sign In', exact: true }).click();
    await expect(page.getByRole('alert')).toContainText('The provided account was created using Google. Please use the Google login flow.');
  });

  test('Login - Using Google with existing password account', async ({ page }) => {
    // Mock all the Google Signin calls
    await mockPublicConfigRequest(page);
    await blockGoogleIdentityScript(page);
    await mockGoogleLibrary(page);
    await mockGoogleAuthRequest(page, {
      ...
      body: JSON.stringify({
        error: {
          code: 'HTTP_409',
          message: 'The provided account was created using a password. Please use the username & password login flow.',
          details: {},
        },
      }),
    });
    ...
    await expect(page.getByRole('alert')).toContainText('The provided account was created using a password. Please use the username & password login flow.');
  });
