...
const rawUrl = process.env.SHU_DATABASE_URL ?? 'postgresql+asyncpg://shu:password@localhost:5432/shu';
...
class DB {
    ...
    async activateUser(email: string): Promise<any>  {
        return await this.db.query('UPDATE users SET is_active=true WHERE email =  $1', [email]);
    };
    ...
    async createUser(email: string, type: UserType): Promise<any>  {
        ...
        `, [
            ...
            null,
            type === UserType.Password ? '$2y$12$OdeaCTGxc3lF7RfGUznJvegcMmnneoQn/RJQmH0Lef3QPFyLv3lsW' : null,
            type
        ]);
    };
