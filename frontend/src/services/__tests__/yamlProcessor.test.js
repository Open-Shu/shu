...
version: 1
`;
      expect(() => YAMLProcessor.parseYAML(invalidYaml)).toThrow(
        /Invalid YAML syntax/,
      );
    });

    it("throws error for empty or null input", () => {
      expect(() => YAMLProcessor.parseYAML("")).toThrow(
        "YAML content must be a non-empty string",
      );
      expect(() => YAMLProcessor.parseYAML(null)).toThrow(
        "YAML content must be a non-empty string",
      );
      expect(() => YAMLProcessor.parseYAML(undefined)).toThrow(
        "YAML content must be a non-empty string",
      );
    });

    it("throws error for non-object YAML", () => {
      expect(() => YAMLProcessor.parseYAML("just a string")).toThrow(
        "YAML must contain a valid object structure",
      );
      expect(() => YAMLProcessor.parseYAML("123")).toThrow(
        "YAML must contain a valid object structure",
      );
...
`;
      ...
      expect(() =>
        YAMLProcessor.convertToExperiencePayload(yamlContent),
...
`;
      ...
      expect(result.errors).toContain(
        "Cron trigger requires cron expression in trigger_config",
      );
    });
