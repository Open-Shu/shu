...
export default function useOAuthAuthorize() {
  ...
  const startAuthorize = useCallback(async ({ provider, scopes, onStart, onDone, onError, onAfterExchange }) => {
    ...
    try {
      ...
      const listener = async (event) => {
        try {
          ...
          try { expectedApi = new URL(getApiBaseUrl()).origin; } catch (_) {}
          if (event.origin !== expectedFrontend && event.origin !== expectedApi) return;
          if (!event.data || typeof event.data !== 'object') return;
          const { provider: evProvider, code } = event.data || {};
          if (String(evProvider || '').toLowerCase() !== prov || !code) return;

          window.removeEventListener('message', listener);
          if (popup && !popup.closed) popup.close();

          // Exchange code for tokens with requested scopes (if provided)
          await hostAuthAPI.exchange({ provider: prov, code, scopes: desiredScopes.length ? desiredScopes : undefined });
