...
describe("downloadHelpers", () => {
  describe("generateSafeFilename", () => {
    it("converts name to safe filename", () => {
      ...
      expect(generateSafeFilename("  Multiple   Spaces  ")).toBe(
        "multiple-spaces",
      );
    });
    ...
    it("handles complex experience names", () => {
      expect(generateSafeFilename("Morning Briefing v2.1")).toBe(
        "morning-briefing-v2-1",
      );
      expect(generateSafeFilename("User Authentication & Authorization")).toBe(
        "user-authentication-authorization",
      );
      expect(generateSafeFilename("API Integration (OAuth2)")).toBe(
