...
import IngestConfigForm from './IngestConfigForm';
...
const ToolRow = ({ connectionId, toolName, description, config, updateToolConfigFn, queryKeyPrefix }) => {
  ...
  const saveMut = useMutation(
    (payload) => updateToolConfigFn(connectionId, toolName, payload).then(extractDataFromResponse),
    {
      onSuccess: () => {
        qc.invalidateQueries([queryKeyPrefix, 'connections']);
        setDirty(false);
      },
    }
  );
  ...
  return (
    <Card variant="outlined" sx={{ mb: 1 }}>
      <CardContent sx={{ py: 1.5, '&:last-child': { pb: 1.5 } }}>
        ...
        {feedEligible && (
          <Box sx={{ mt: 1.5 }}>
            <IngestConfigForm config={ingestConfig} onChange={handleIngestChange} />
          </Box>
        )}
...
export default function ToolConfigPanel({ connection, updateToolConfigFn, queryKeyPrefix = 'mcp' }) {
        ...
        <Stack spacing={0}>
          {discoveredTools.map((tool) => (
            <ToolRow
              ...
              updateToolConfigFn={updateToolConfigFn}
