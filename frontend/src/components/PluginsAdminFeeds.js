...
import { extractDataFromResponse, extractItemsFromResponse, formatError, authAPI, knowledgeBaseAPI } from '../services/api';
...
export default function PluginsAdminFeeds() {
  ...
  const kbsQ = useQuery(['kbs','list'], () => knowledgeBaseAPI.list().then(extractItemsFromResponse), { staleTime: 10000 });
  ...
  const runPendingMut = useMutation(
    (opts) => schedulesAPI.runPending(opts).then(extractDataFromResponse)
  );
  ...
  const handleRunNow = (row) => {
    ...
    schedulesAPI.runNow(row.id).then(extractDataFromResponse).then(() => {
      runPendingMut.mutate({ limit: 1, schedule_id: row.id }, { onSuccess: () => qc.invalidateQueries(['executions','schedule', row.id]) });
    });
  };
  ...
  return (
    <Box p={3}>
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
        ...
        <Stack direction="row" spacing={1}>
          ...
          <Tooltip title="Enqueue due feeds">
            <span>
              <Button variant="outlined" onClick={() => runDueMut.mutate()} disabled={runDueMut.isLoading}>Run Due</Button>
            </span>
          </Tooltip>
          <Tooltip title="Run pending executions (development)">
            <span>
              <Button variant="outlined" onClick={() => runPendingMut.mutate({ limit: 5 })} disabled={runPendingMut.isLoading}>Run Pending</Button>
            </span>
          </Tooltip>
      ...
      <FeedTable
        ...
        showKbColumn
        showRunsButton
        onChangeKb={(row, kbId) => patchMut.mutate({ id: row.id, payload: { params: { ...(row.params || {}), kb_id: kbId || null } } })}
      ...
      <FeedCreateDialog open={createOpen} onClose={() => setCreateOpen(false)} onCreated={() => qc.invalidateQueries(['schedules','list'])} />
