...
export default function TemplateVariableHints({ steps = [], includePreviousRun = false, onInsert }) {
    ...
    const contextVariables = [
        { label: 'now', variable: 'now' },
    ];

    if (includePreviousRun) {
        contextVariables.push(
            { label: 'previous_run.result_content', variable: 'previous_run.result_content' },
            { label: 'previous_run.step_outputs', variable: 'previous_run.step_outputs' },
        );
    }
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                ...
                {contextVariables.map(v => (
                    <Chip
                        ...
                        label={v.label}
                        size="small"
                        icon={v.variable.startsWith('previous') ? <HistoryIcon sx={{ fontSize: 14 }} /> : <TimeIcon sx={{ fontSize: 14 }} />}
                        onClick={() => handleInsert(v.variable)}
                        sx={{
                            cursor: 'pointer',
                            fontSize: '0.7rem',
