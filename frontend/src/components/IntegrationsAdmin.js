...
const McpConnectionCard = ({ connection, onSync, onEdit, onDelete, onToggleEnabled, isSyncing }) => {
    ...
    >
      <CardContent>
        ...
        <ToolConfigPanel
          ...
          updateToolConfigFn={mcpAPI.updateToolConfig}
          queryKeyPrefix="mcp"
        />
      </CardContent>
    </Card>
...
const ApiConnectionCard = ({ connection, onSync, onDelete, onToggleEnabled, isSyncing }) => {
  const statusColor = STATUS_COLORS[connection.status] || 'default';
  const toolCount = connection.tool_count || 0;
      ...
      <CardContent>
        <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
          <Box sx={{ minWidth: 0, flex: 1 }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
              {connection.base_url || connection.url}
            </Typography>
            {connection.last_error && (
              <Typography variant="caption" color="error.main">
                Last error: {connection.last_error}
          ...
          <Stack direction="row" alignItems="center" spacing={1}>
            <Tooltip title={connection.enabled ? 'Disable integration' : 'Enable integration'}>
              <Switch
                checked={connection.enabled}
                onChange={() => onToggleEnabled(connection.id, !connection.enabled)}
                size="small"
        ...
        <ToolConfigPanel
          connection={connection}
          updateToolConfigFn={apiIntegrationsAPI.updateToolConfig}
          queryKeyPrefix="api-integrations"
        />
      </CardContent>
    </Card>
  );
...
const ApiTab = () => {
  ...
  const { data, isLoading, error, isFetching, refetch } = useQuery(
    ...
    () => apiIntegrationsAPI.listConnections().then(extractDataFromResponse),
    { refetchOnWindowFocus: false }
  );

  const connections = data?.items || [];

  const deleteMut = useMutation((id) => apiIntegrationsAPI.deleteConnection(id).then(extractDataFromResponse), {
    onSuccess: () => qc.invalidateQueries(['api-integrations', 'connections']),
    onError: (err) => {
      window.alert(`Delete failed: ${formatError(err)}`);
    },
  });

  const syncMut = useMutation((id) => apiIntegrationsAPI.syncConnection(id).then(extractDataFromResponse), {
    onSuccess: () => {
  ...
  const handleToggleEnabled = (id, enabled) => {
    apiIntegrationsAPI
      .updateConnection(id, { enabled })
      .then(() => qc.invalidateQueries(['api-integrations', 'connections']));
  };

  const handleDelete = (connection) => {
    if (!window.confirm(`Delete API integration "${connection.name}"? This cannot be undone.`)) {
      return;
    }
    deleteMut.mutate(connection.id);
  };
  ...
  return (
    <Box>
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={3}>
        <Typography variant="subtitle1" sx={{ fontWeight: 600 }}>
          {connections.length} integration{connections.length !== 1 ? 's' : ''}
        </Typography>
        <Stack direction="row" spacing={1}>
          <Button variant="contained" startIcon={<AddIcon />} onClick={() => setDialogOpen(true)}>
            Add Integration
          </Button>
          <Tooltip title="Refresh integration list">
            <span>
              <Button variant="outlined" startIcon={<RefreshIcon />} onClick={() => refetch()} disabled={isFetching}>
                Refresh
              </Button>
        ...
        <Stack spacing={2}>
          {connections.length === 0 ? (
            ...
            >
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                Add an API integration via YAML to get started
              </Typography>
              <Button variant="contained" startIcon={<AddIcon />} onClick={() => setDialogOpen(true)}>
                Add Integration
              </Button>
      ...
      <ApiIntegrationDialog open={dialogOpen} onClose={() => setDialogOpen(false)} />
    </Box>
  );
};
...
const IntegrationsAdmin = () => {
  ...
  return (
    <Box p={3}>
      <PageHelpHeader
        title="Integrations"
        description="Connect external MCP servers or API integrations to expose their tools as Shu plugins. Tools can be used in chat or scheduled as ingest feeds."
        icon={<HubIcon />}
        tips={[
          ...
          'API integrations are configured via YAML definitions',
        ]}
      />

      <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 3 }}>
        <Tabs value={activeTab} onChange={(_, newValue) => setActiveTab(newValue)}>
          <Tab icon={<HubIcon />} iconPosition="start" label="MCP Servers" />
          <Tab icon={<ApiIcon />} iconPosition="start" label="API Integrations" />
        </Tabs>
      </Box>
