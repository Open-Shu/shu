...
export default function ExperienceStepBuilder({ steps, onChange }) {
  // Fetch plugins
  const pluginsQuery = useQuery(['plugins', 'list'], () => pluginsAPI.list().then(extractDataFromResponse), {
    staleTime: 30000,
  });

  // Fetch knowledge bases
  const kbQuery = useQuery(['kbs', 'list'], () => knowledgeBaseAPI.list().then(extractItemsFromResponse), {
    staleTime: 30000,
  });
  ...
  return (
    <Box>
      ...
      <Stack spacing={2} sx={{ mb: 2 }}>
        {steps.length === 0 ? (
          <Alert severity="info">No steps configured. Add steps to gather data for your experience.</Alert>
        ) : (
      ...
      <Stack direction="row" spacing={1}>
        <Button variant="outlined" size="small" startIcon={<PluginIcon />} onClick={() => handleAddStep('plugin')}>
          Add Plugin Step
        </Button>
