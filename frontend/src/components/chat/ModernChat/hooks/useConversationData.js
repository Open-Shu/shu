...
import { chatAPI, extractDataFromResponse, formatError, modelConfigAPI } from '../../../../services/api';
...
const useConversationData = ({ summaryQuery, onQueryError, onMutationError, markFreshConversation }) => {
  const queryClient = useQueryClient();
  const [lastCreatedConversation, setLastCreatedConversation] = useState(null);

  const conversationListParams = useMemo(() => buildConversationListParams(summaryQuery), [summaryQuery]);

  const conversationQueryKey = useMemo(() => ['conversations', summaryQuery || ''], [summaryQuery]);
  ...
  const availableModelConfigs = useMemo(() => filterAvailableModelConfigs(modelConfigs), [modelConfigs]);

  const createConversationMutation = useMutation((data) => chatAPI.createConversationWithModelConfig(data), {
    onSuccess: (response) => {
