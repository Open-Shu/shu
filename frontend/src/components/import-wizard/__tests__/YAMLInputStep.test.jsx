...
describe('YAMLInputStep', () => {
  ...
  test('renders YAML input step with title and description', () => {
    ...
    expect(screen.getByText(/Paste or edit your experience YAML configuration/)).toBeInTheDocument();
  ...
  test('displays pre-populated YAML indicator when provided', () => {
    ...
    render(<YAMLInputStep {...defaultProps} prePopulatedYAML={prePopulatedYAML} />);

    expect(screen.getByText(/This YAML has been pre-populated from the Quick Start guide/)).toBeInTheDocument();
  });
  ...
  test('calls onYAMLChange when pre-populated YAML is provided', () => {
    ...
    render(<YAMLInputStep {...defaultProps} onYAMLChange={onYAMLChange} prePopulatedYAML={prePopulatedYAML} />);
  ...
  test('displays validation errors when YAML is invalid', async () => {
    ...
    render(
      <YAMLInputStep {...defaultProps} yamlContent="invalid: yaml: content:" onValidationChange={onValidationChange} />
    );
  ...
  test('shows empty state message when no content', async () => {
    ...
    render(<YAMLInputStep {...defaultProps} yamlContent="" onValidationChange={onValidationChange} />);
  ...
  test('displays character count for non-empty content', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(`(${yamlContent.length} characters)`)).toBeInTheDocument();
    });
  });
  ...
  test('clears pre-populated flag when user edits content', () => {
    ...
    const { rerender } = render(
      <YAMLInputStep {...defaultProps} onYAMLChange={onYAMLChange} prePopulatedYAML={prePopulatedYAML} />
    );

    // Initially shows pre-populated indicator
    expect(screen.getByText(/This YAML has been pre-populated/)).toBeInTheDocument();
    ...
    expect(screen.queryByText(/This YAML has been pre-populated/)).not.toBeInTheDocument();
  });
  ...
  test('handles validation errors gracefully', async () => {
    ...
    render(<YAMLInputStep {...defaultProps} yamlContent="some content" onValidationChange={onValidationChange} />);

    await waitFor(() => {
      expect(screen.getByText('Invalid YAML')).toBeInTheDocument();
      expect(screen.getByText('Validation error: Validation service error')).toBeInTheDocument();
    });
