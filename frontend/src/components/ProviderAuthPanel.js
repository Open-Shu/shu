...
export default function ProviderAuthPanel({
  ...
  const opAuthSpec = useMemo(() => {
    try {
      return pluginDef?.op_auth && pluginDef.op_auth[opKey];
    } catch (_) {
      return null;
    }
  }, [pluginDef, opKey]);
  ...
  useEffect(() => {
    ...
    if (provider === 'google') {
      hostAuthAPI
        ...
        .then((data) => {
          if (mounted) {
            setGoogleStatus((data && data.google) || {});
          }
        })
        .catch(() => {
          if (mounted) {
  ...
  const runProbe = async () => {
    if (!provider || !Array.isArray(scopes) || scopes.length === 0) {
      return;
    }
    try {
      ...
      {showIdentityGate && (
        <>
          <IdentityGate
            ...
            identityStatusProps={{
              useServerUnionForAuthorize: true,
