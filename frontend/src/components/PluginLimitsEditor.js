      ...
      pluginsAPI
        .setLimits(name, {
          rate_limit_user_requests:
            form.rate_limit_user_requests === ""
              ? undefined
              : form.rate_limit_user_requests,
          rate_limit_user_period:
            form.rate_limit_user_period === ""
              ? undefined
              : form.rate_limit_user_period,
          quota_daily_requests:
            form.quota_daily_requests === ""
              ? undefined
              : form.quota_daily_requests,
          quota_monthly_requests:
            form.quota_monthly_requests === ""
              ? undefined
              : form.quota_monthly_requests,
          // Provider caps
          provider_name:
            form.provider_name === "" ? undefined : form.provider_name,
          provider_rpm:
            form.provider_rpm === "" ? undefined : form.provider_rpm,
          provider_window_seconds:
            form.provider_window_seconds === ""
              ? undefined
              : form.provider_window_seconds,
          provider_concurrency:
            form.provider_concurrency === ""
              ? undefined
              : form.provider_concurrency,
        })
      ...
      <CardContent>
        <Stack spacing={2}>
          ...
          <Typography variant="body2" color="text.secondary">
            These overrides apply per plugin/version and per user. Leave a field
            blank to keep the existing value. Set 0 to disable a limit (e.g.,
            disable quotas).
          </Typography>
          {isLoading && (
          ...
          <Typography variant="body2" color="text.secondary">
            These limits are enforced at the provider/model level and shared
            across all plugins using the same provider_name.
          </Typography>
