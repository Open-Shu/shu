...
describe('RecurringScheduleBuilder', () => {
  ...
  test('handles empty values', () => {
    render(<RecurringScheduleBuilder value={{}} onChange={jest.fn()} validationErrors={{}} />);

    expect(screen.getByTestId('mock-cron-input')).toHaveValue('0 9 * * *'); // default value
  });

  test('displays validation errors', () => {
    render(<RecurringScheduleBuilder {...defaultProps} validationErrors={{ cron: 'Invalid cron expression' }} />);
  ...
  test('displays timezone explanation text', () => {
    ...
    expect(
      screen.getByText(/Timezones ensure your scheduled experiences run at the correct local time/)
    ).toBeInTheDocument();
  });
  ...
  test('updates preview in real-time when cron changes', async () => {
    ...
    rerender(
      <RecurringScheduleBuilder {...defaultProps} value={{ cron: '0 10 * * *', timezone: 'America/New_York' }} />
    );
  ...
  describe('Complex Cron Expression Fallback', () => {
    ...
    test('handles cron expression updates that become complex', async () => {
      ...
      rerender(
        <RecurringScheduleBuilder {...defaultProps} value={{ cron: '*/5 * * * *', timezone: 'America/New_York' }} />
      );
    ...
    test('clears warning when cron expression changes from complex to simple', async () => {
      ...
      rerender(
        <RecurringScheduleBuilder {...defaultProps} value={{ cron: '0 9 * * *', timezone: 'America/New_York' }} />
      );
  ...
  describe('Edge Case Handling', () => {
    ...
    test('displays warning for day 29 scheduled across all months (leap year)', async () => {
      ...
      expect(
        screen.getByText(/Day 29 is scheduled but does not exist in February \(except leap years\)/i)
      ).toBeInTheDocument();
    });
    ...
    test('displays DST transition note in schedule preview', async () => {
      const mockPreview = {
        ...
        nextExecutions: ['Sunday, March 9, 2026 at 2:00 AM EDT (near DST spring forward)'],
        executionDates: [],
      };
