...
describe("RecurringScheduleBuilder", () => {
  ...
  test('applies weekday schedule when corresponding "Use this" button is clicked', () => {
    const mockOnChange = jest.fn();
    render(
      <RecurringScheduleBuilder {...defaultProps} onChange={mockOnChange} />,
    );
  ...
  test("displays timezone explanation text", () => {
    ...
    expect(
      screen.getByText(
        /Timezones ensure your scheduled experiences run at the correct local time/,
      ),
    ).toBeInTheDocument();
  });
  ...
  test("displays schedule preview when cron and timezone are set", async () => {
    ...
    expect(
      screen.getByText("Tuesday, January 14, 2026 at 9:00 AM EST"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Wednesday, January 15, 2026 at 9:00 AM EST"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Thursday, January 16, 2026 at 9:00 AM EST"),
    ).toBeInTheDocument();
  });
  ...
  test("displays error message when preview generation fails", async () => {
    ...
    await waitFor(
      () => {
        expect(
          screen.getByText("Invalid cron expression format"),
        ).toBeInTheDocument();
      },
      { timeout: 500 },
    );
  ...
  test("handles missing timezone gracefully", () => {
    ...
    expect(
      screen.getByText("Configure a schedule to see preview"),
    ).toBeInTheDocument();
  });
  ...
  describe("Advanced Mode Toggle", () => {
    ...
    test("renders advanced mode toggle button", () => {
      ...
      expect(
        screen.getByRole("button", { name: /advanced mode/i }),
      ).toBeInTheDocument();
    });
    ...
    test("switches to advanced mode when toggle is clicked", () => {
      ...
      expect(
        screen.getByRole("button", { name: /visual builder/i }),
      ).toBeInTheDocument();
    });
    ...
    test("hides toggle button when cron expression is complex", () => {
      ...
      expect(
        screen.queryByRole("button", { name: /visual builder/i }),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByRole("button", { name: /advanced mode/i }),
      ).not.toBeInTheDocument();

      // Should show warning
      expect(
        screen.getByText(/too complex for the visual builder/i),
      ).toBeInTheDocument();
    });
    ...
    test("clears complex expression warning when dismissing", () => {
      ...
      expect(
        screen.queryByText(/too complex for the visual builder/i),
      ).not.toBeInTheDocument();
  ...
  describe("Complex Cron Expression Fallback", () => {
    ...
    test("handles cron expression updates that become complex", async () => {
      const { rerender } = render(
        <RecurringScheduleBuilder {...defaultProps} />,
      );
    ...
    test("clears warning when cron expression changes from complex to simple", async () => {
      ...
      const { rerender } = render(
        <RecurringScheduleBuilder {...complexCronProps} />,
      );
  ...
  describe("Edge Case Handling", () => {
    ...
    test("displays warning for day 31 scheduled across all months", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Schedule Considerations:/i),
        ).toBeInTheDocument();
      });

      // Should show warning about day 31
      expect(
        screen.getByText(/Day 31 is scheduled but only exists in 7 months/i),
      ).toBeInTheDocument();
    });
    ...
    test("displays warning for day 30 scheduled across all months", async () => {
      ...
      expect(
        screen.getByText(/Day 30 is scheduled but does not exist in February/i),
      ).toBeInTheDocument();
    });
    ...
    test("displays warning for day 29 scheduled across all months (leap year)", async () => {
      ...
      expect(
        screen.getByText(
          /Day 29 is scheduled but does not exist in February \(except leap years\)/i,
        ),
      ).toBeInTheDocument();
    });
    ...
    test("displays error for day 31 scheduled in February", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/Invalid explicit day of month definition/i),
        ).toBeInTheDocument();
      });
    });
    ...
    test("does not display warnings for valid day-of-month selections", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.queryByText(/Schedule Considerations:/i),
          ).not.toBeInTheDocument();
        },
        { timeout: 500 },
      );
    ...
    test("displays DST transition note in schedule preview", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/near DST spring forward/i),
          ).toBeInTheDocument();
        },
        { timeout: 500 },
      );
    ...
    test("handles multiple validation warnings", async () => {
      ...
      expect(
        screen.getByText(/Day 29 is scheduled but does not exist in February/i),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Day 30 is scheduled but does not exist in February/i),
      ).toBeInTheDocument();
