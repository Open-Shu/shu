  ...
  const validateFile = useCallback(
    (file) => {
      ...
      if (file.size > maxSizeBytes) {
        return {
          valid: false,
          error: `Too large: ${formatFileSize(file.size)} > ${formatFileSize(maxSizeBytes)}`,
        };
      }
      return { valid: true };
    },
    ...
    <Box>
      ...
      >
        ...
        <Typography variant="body2" color="text.secondary">
          {allowedTypes.length > 0 ? `Allowed: ${allowedTypes.join(', ')}` : 'All file types'} | Max:{' '}
        ...
        <List dense sx={{ mt: 2 }}>
          {pendingFiles.map((item, index) => (
            <ListItem
              key={index}
              sx={{
                bgcolor: item.valid ? 'transparent' : alpha(theme.palette.error.main, 0.08),
                borderRadius: 1,
              }}
            >
          ...
          {uploadResults.map((result, index) => (
            ...
            >
              ...
              <ListItemText
                ...
                secondary={
                  result.success ? `${result.word_count || 0} words, ${result.chunk_count || 0} chunks` : result.error
                }
                secondaryTypographyProps={{
