...
export default function FeedDialog({
  ...
  const schedule =
    isEdit && scheduleQuery.data ? scheduleQuery.data : scheduleProp;
  ...
  const [pluginName, setPluginName] = useState(
    presetPlugin || schedule?.plugin_name || "",
  );
  ...
  const kbQ = useQuery(
    ...
    { enabled: open && !lockedKbId, staleTime: 10000 },
  );
  ...
  const plugins = useMemo(
    () => (Array.isArray(pluginsQ.data) ? pluginsQ.data : []),
    [pluginsQ.data],
  );
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      ...
      try {
        const m =
          schedule?.params?.__host?.ocr?.mode ||
          schedule?.params?.__host?.ocr_mode ||
          "auto";
        setOcrMode(String(m));
      } catch (_) {
        setOcrMode("auto");
  ...
  useEffect(() => {
    ...
    const first = plugins.find(
      (t) => Array.isArray(t.allowed_feed_ops) && t.allowed_feed_ops.length > 0,
    );
    if (first) {
  ...
  useEffect(() => {
    ...
    if (selectedPlugin.input_schema) {
      ...
      let merged = isEdit
        ? { ...defaults, ...(schedule?.params || {}) }
        : defaults;
      try {
        const allowedFeedOps = Array.isArray(selectedPlugin?.allowed_feed_ops)
          ? selectedPlugin.allowed_feed_ops
          : [];
    ...
    setIdentitiesOk(
      !(
        Array.isArray(selectedPlugin?.required_identities) &&
        selectedPlugin.required_identities.length > 0
      ),
    );
  ...
  const defaultFeedOp = selectedPlugin?.default_feed_op || null;
  const opForOcr = useMemo(
    () => String(values?.op || defaultFeedOp || "").toLowerCase(),
    [values, defaultFeedOp],
  );
  const hasOcrCap = useMemo(
    () =>
      Array.isArray(selectedPlugin?.capabilities) &&
      selectedPlugin.capabilities.includes("ocr"),
    [selectedPlugin],
  );
  ...
  const updateMut = useMutation(
    (payload) =>
      schedulesAPI.update(schedule.id, payload).then(extractDataFromResponse),
    {
      onSuccess: () => {
        qc.invalidateQueries(["schedules", "list"]);
        if (onClose) {
  ...
  const renderSchemaForm = useMemo(() => {
    ...
    const allowedOps = Array.isArray(selectedPlugin?.allowed_feed_ops)
      ? selectedPlugin.allowed_feed_ops
      : [];
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle>
        {isEdit ? "Edit Plugin Feed" : "Create Plugin Feed"}
      </DialogTitle>
              ...
              <FormControl fullWidth>
                ...
                >
                  {plugins
                    .filter(
                      (t) =>
                        Array.isArray(t.allowed_feed_ops) &&
                        t.allowed_feed_ops.length > 0,
                    )
                    .map((t) => (
          ...
          <Grid item xs={12} md={6}>
            {lockedKbId && !isEdit ? (
              <TextField
                ...
                value={
                  (kbLockedQ.data &&
                    (kbLockedQ.data.name || kbLockedQ.data.id)) ||
                  lockedKbId
                }
                disabled
              />
          ...
          <Grid item xs={12} md={6}>
            <FormControl fullWidth>
              ...
              >
                ...
                {!userOptions.some(
                  (o) => String(o.id) === String(ownerUserId),
                ) &&
                  ownerUserId && (
          ...
          <Box mt={2}>
            {String(process.env.NODE_ENV) === "development" && (
              <FormControlLabel
                control={
                  <Checkbox
                    checked={devDiagnostics}
                    onChange={(e) => setDevDiagnostics(e.target.checked)}
                  />
                }
            ...
            <Typography variant="caption" color="text.secondary">
              Resetting the cursor clears the incremental checkpoint for this
              feed and KB. The next run will perform a full discovery and may
            ...
            <Box mt={2}>
              ...
              <FormControl fullWidth>
                ...
                >
                  ...
                  <MenuItem value="auto">
                    Auto — OCR PDFs/images; skip text files
                  </MenuItem>
                  <MenuItem value="fallback">
                    Fallback — Try text first; OCR if empty
                  </MenuItem>
