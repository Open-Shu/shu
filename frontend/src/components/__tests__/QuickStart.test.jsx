...
describe("QuickStart Component - Experiences Card", () => {
  ...
  test("Experiences card contains correct title, description, and icon", async () => {
    ...
    expect(
      screen.getByText(
        /Create automated workflows that combine plugins, knowledge bases, and AI synthesis/,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Build signature experiences like Morning Briefing/),
    ).toBeInTheDocument();

    // Check that the card is clickable (has CardActionArea)
    const experiencesCard = screen
      .getByText("Experiences")
  ...
  test("Experiences card does not show completion status when experience_created is false", async () => {
    ...
    const experiencesCardHasDone = doneChips.some(
      (chip) => experiencesCard && experiencesCard.contains(chip),
    );
  ...
  test("Experiences card uses same SectionCard component and design patterns", async () => {
    ...
    const cardActionArea = experiencesCard.querySelector(
