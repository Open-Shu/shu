...
jest.mock("../import-wizard/YAMLInputStep", () => {
  ...
  return function MockYAMLInputStep({
    yamlContent,
    onYAMLChange,
    onValidationChange,
    prePopulatedYAML,
    ...
    return mockReact.createElement(
      ...
      mockReact.createElement(
        "p",
        null,
        `Pre-populated: ${prePopulatedYAML ? "Yes" : "No"}`,
      ),
      mockReact.createElement(
        "button",
        {
...
jest.mock("../import-wizard/PlaceholderFormStep", () => {
  const mockReact = require("react");
  return function MockPlaceholderFormStep({
    placeholders,
    onValidationChange,
  }) {
    mockReact.useEffect(() => {
...
jest.mock("../import-wizard/ExperienceCreationStep", () => {
  const mockReact = require("react");
  return function MockExperienceCreationStep({
    isCreating,
    error,
    success,
    experienceId,
    ...
    return mockReact.createElement(
      ...
      success &&
        mockReact.createElement(
          "p",
          null,
          `Success! Experience ID: ${experienceId}`,
        ),
    );
  };
});
...
describe("ImportExperienceWizard", () => {
  ...
  test("handles close button click", () => {
    ...
    renderWithQueryClient(
      <ImportExperienceWizard {...defaultProps} onClose={onClose} />,
    );
  ...
  test("maintains stepper state correctly", () => {
    ...
    const stepLabels = screen.getAllByText(
      /^(YAML Input|Configure Values|Create Experience)$/,
    );
