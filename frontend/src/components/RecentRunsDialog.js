  ...
  return (
    <Dialog open={!!open} onClose={onClose} maxWidth="md" fullWidth>
      ...
      <DialogContent dividers>
        {!schedule ? null : (
          <>
            {runsQ.isLoading && (<Box display="flex" alignItems="center" gap={1}><CircularProgress size={18} /> Loading…</Box>)}
            ...
            {Array.isArray(runsQ.data) && runsQ.data.map((r) => (
              <Box key={r.id} sx={{ mb: 1, p: 1, border: '1px solid #eee', borderRadius: 1 }}>
                <Stack direction="row" spacing={1} alignItems="center" justifyContent="space-between">
                  <Stack direction="row" spacing={1} alignItems="center">
                    <Chip size="small" label={r.status} color={r.status === 'completed' ? 'success' : (r.status === 'failed' ? 'error' : 'default')} />
                    <Typography variant="body2">{formatLocalShort(r.completed_at || r.started_at || r.created_at)}</Typography>
                  </Stack>
                  <Stack direction="row" spacing={1}>
                    {r.error && <Typography variant="body2" color="error">{String(r.error).slice(0, 140)}</Typography>}
                  </Stack>
                </Stack>
                {r.result && (
                  <Box sx={{ mt: 1 }}>
      ...
      <DialogActions>
        <Button onClick={() => schedule && runsQ.refetch()} disabled={!schedule || runsQ.isFetching}>Refresh</Button>
