...
export default function ConnectedAccountsPage() {
  ...
  const pluginsQ = useQuery(["plugins", "list"], () =>
    api.get("/plugins").then(extractDataFromResponse),
  );
  ...
  const { requiredIdentities, scopeMapByProvider } = useMemo(() => {
    ...
    for (const p of plugins) {
      ...
      const reqIds = Array.isArray(p?.required_identities)
        ? p.required_identities
        : [];
    ...
    if (byProv["google"]) {
      ...
      const hasAnyGmail = unionScopes.some((s) =>
        s.includes("https://www.googleapis.com/auth/gmail."),
      );
      const hasGmailReadonly = unionScopes.includes(
        "https://www.googleapis.com/auth/gmail.readonly",
      );
      const hasGmailModify = unionScopes.includes(
        "https://www.googleapis.com/auth/gmail.modify",
      );
      if (hasAnyGmail && !hasGmailReadonly && !hasGmailModify) {
        byProv["google"].scopes.add(
          "https://www.googleapis.com/auth/gmail.readonly",
        );
      }
    }
  ...
  const providers = useMemo(
    () => Object.keys(scopeMapByProvider || {}),
    [scopeMapByProvider],
  );
  ...
  const subsQueriesArr = useQueries(
    (providers || []).map((prov) => ({
      ...
      queryFn: () =>
        hostAuthAPI.listSubscriptions(prov).then(extractDataFromResponse),
      enabled: !!prov,
    })),
  );
  const subsQueries = useMemo(
    () =>
      Object.fromEntries(
        (providers || []).map((prov, idx) => [prov, subsQueriesArr[idx]]),
      ),
  ...
  const consentQueries = useMemo(
    () =>
      Object.fromEntries(
        (providers || []).map((prov, idx) => [prov, consentQueriesArr[idx]]),
      ),
  ...
  const subscribeMut = useMutation(
    ({ provider, plugin }) =>
      hostAuthAPI.subscribe(provider, plugin).then(extractDataFromResponse),
    {
      onMutate: async (vars) => {
        ...
        qc.setQueryData(key, (old) => {
          ...
          return exists
            ? old
            : { items: [...items, { plugin_name: vars.plugin }] };
        });
        return { previous };
      },
      onError: (e, vars, ctx) => {
        if (ctx?.previous)
          qc.setQueryData(
            ["hostAuth", "subscriptions", vars.provider],
            ctx.previous,
          );
        setSnack({
          open: true,
  ...
  const unsubscribeMut = useMutation(
    ({ provider, plugin }) =>
      hostAuthAPI.unsubscribe(provider, plugin).then(extractDataFromResponse),
    {
      onMutate: async (vars) => {
        await qc.cancelQueries(["hostAuth", "subscriptions", vars.provider]);
        const key = ["hostAuth", "subscriptions", vars.provider];
    ...
    for (const p of plugins) {
      ...
      if (opAuth && typeof opAuth === "object") {
        for (const key of Object.keys(opAuth)) {
          ...
          if (prov)
            scopesByProv[prov] = Array.from(
              new Set([...(scopesByProv[prov] || []), ...sc.map(String)]),
            );
        }
      }
      for (const ri of reqIds) {
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        Manage provider connections, plugin subscriptions, and secrets. The
        server requests consent for the union of scopes across your subscribed
        plugins.
      </Typography>
        ...
        {providers.length === 0 ? (
          ...
          <Box>
            {providers.map((prov) => {
              ...
              const subs = Array.isArray(subsQ?.data?.items)
                ? subsQ.data.items
                : [];
                  ...
                  {items.length === 0 ? (
                    ...
                    <Stack>
                      {items.map((pl) => {
                        ...
                        const pluginScopes =
                          pluginsByProviderScopes?.[prov]?.[pl.name] || [];
                        const labelNode = (
                          <Tooltip
                            title={
                              pluginScopes.length ? pluginScopes.join("\n") : ""
                            }
                          >
                            <span>{pl.label}</span>
                          </Tooltip>
