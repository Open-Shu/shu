...
export default function IdentityStatus({ requiredIdentities = [], onStatusChange, showManageLink = true, useServerUnionForAuthorize = false, hideConnectButton = false, showDisconnect = true, authorizingMap = null, setAuthorizingMap = null }) {
  ...
  const consentQueriesArr = useQueries((useServerUnionForAuthorize ? providers : []).map((prov) => ({
  ...
  const consentScopesByProvider = useMemo(() => {
    if (!useServerUnionForAuthorize) return {};
    const map = {};
  ...
  useEffect(() => {
    if (onStatusChange) onStatusChange(allConnected, statusQ.data || {});
            ...
            <Stack key={p} direction="row" spacing={1} alignItems="center">
              ...
              {needReauth && !hideConnectButton && (() => {
                ...
                return (
                  <Tooltip title={tooltipText}>
                    <span>
                      <Button size="small" variant="outlined" disabled={!!authorizing[p] || displayScopes.length === 0} onClick={() => handleConnect(p, displayScopes)}>
              ...
              {needReauth && showManageLink && (
                <Tooltip title="Open the Connected Accounts page to review or manage providers">
                  <Button size="small" variant="text" onClick={() => navigate('/settings/connected-accounts')}>Manage in Connected Accounts</Button>
                </Tooltip>
              )}
                ...
                <Tooltip title="Removes stored tokens and identities for this provider">
                  <span>
                    <Button
                      ...
                      color="warning"
                      onClick={async () => {
                        const ok = window.confirm(`Disconnect ${p}? This will remove stored tokens and identities for this provider.`);
                        if (!ok) return;
                        try {
