        ...
        return {
            ...
            "properties": {
                ...
                "customer_id": {
                    "type": "string",
                    "x-ui": {
                        "help": "Customer ID for the hold"
                    }
                },
                "customer_name": {
                    "type": "string",
                    "x-ui": {
                        "help": "Customer name for the hold"
                    }
                },
                "customer_tier": {
                    "type": "string",
                ...
                "arrival_time": {
                    ...
                    "format": "date-time",
                    "x-ui": {
                        "help": "Customer arrival time (ISO 8601 format)"
                    }
                },
                "notes": {
                    "type": "string",
    ...
    def get_output_schema(self) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "properties": {
                "notification": {
                    **notification_schema,
                    "description": "Hold notification details"
                },
                "tailor_availability": {
                    **availability_schema,
                    "description": "Tailor availability information"
                },
                "message": {
                    "type": "string",
        ...
        if op == "add_hold":
            ...
            if not customer_id:
                return ToolResult.err(
                    "customer_id is required for add_hold operation",
            ...
            if arrival_time_str:
                ...
                arrival_time = datetime.now(timezone.utc).replace(tzinfo=timezone(timedelta(hours=4)))
            
            # Generate notification ID
            global _notification_counter
            _notification_counter += 1
            notification_id = f"TAILOR-{datetime.now(timezone.utc).strftime('%Y%m%d')}-{_notification_counter:03d}"
            ...
            return ToolResult.ok(
                data={"tailor_availability": tailor_availability},
                diagnostics=["Demo mode: using synthesized data"]
            )
