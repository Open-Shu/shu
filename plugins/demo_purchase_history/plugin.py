...
class ToolResult:
    ...
    def err(cls, message: str, code: str = "error", details: dict[str, Any] | None = None):
        ...
        """
        error_dict = {"message": message, "code": code}
        if details:
...
class DemoPurchaseHistoryPlugin:
    ...
    def get_schema(self) -> dict[str, Any] | None:
        ...
        return {
            "type": "object",
            "properties": {
                "op": {"type": "string", "enum": ["get"], "default": "get", "description": "Operation to perform"},
                "player_id": {
                    "type": "string",
    ...
    def get_output_schema(self) -> dict[str, Any] | None:
        ...
        return {
            ...
            "properties": {
                ...
                "time_window_minutes": {"type": "integer", "description": "Time window in minutes"},
                "purchases": {
                    "type": "array",
                    ...
                    "items": {
                        "type": "object",
                        "properties": {
                            "timestamp": {"type": "string", "format": "date-time", "description": "Purchase timestamp"},
                            "item": {"type": "string", "description": "Item name"},
                            "quantity": {"type": "integer", "description": "Quantity purchased"},
                            "alcohol_content": {"type": "number", "description": "Alcohol content percentage"},
                            "volume_oz": {"type": "number", "description": "Volume in fluid ounces"},
                            "server": {"type": "string", "description": "Server name"},
                        },
                    },
                },
                "alert_threshold": {"type": "number", "description": "BAC alert threshold"},
                "cutoff_threshold": {"type": "number", "description": "BAC cutoff threshold"},
            },
        }
    ...
    async def execute(self, params: dict[str, Any], context: Any, host: Any) -> ToolResult:
        ...
        if not player_id:
            return ToolResult.err(message="player_id is required for get operation", code="missing_parameter")

        # Validate operation
        if op != "get":
            return ToolResult.err(message=f"Unknown operation: {op}", code="invalid_operation")

        # Subtask 6.2: Purchase data retrieval
        # Look up player in DEMO_PURCHASE_HISTORY
        if player_id not in DEMO_PURCHASE_HISTORY:
            return ToolResult.err(message=f"No purchase history found for player: {player_id}", code="player_not_found")
        ...
        if location:
            if location not in player_data:
                return ToolResult.err(message=f"No purchases found at location: {location}", code="location_not_found")
