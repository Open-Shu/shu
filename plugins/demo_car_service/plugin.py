    ...
    def get_schema(self) -> dict[str, Any] | None:
        ...
        return {
            ...
            "properties": {
                ...
                "customer_id": {"type": "string", "x-ui": {"help": "Customer ID for booking"}},
                "customer_name": {"type": "string", "x-ui": {"help": "Customer name for booking"}},
                "car_delivery_location": {"type": "string", "x-ui": {"help": "Pickup location address"}},
                "dropoff_location": {"type": "string", "x-ui": {"help": "Drop-off location address"}},
                "passengers": {
                    "type": "integer",
                    "minimum": 1,
                    "maximum": 10,
                    "default": 1,
                    "x-ui": {"help": "Number of passengers"},
                },
                "car_type": {
                    "type": "string",
                    "enum": ["mercedes_s_class", "rolls_royce_phantom", "any"],
                    "default": "any",
                    "x-ui": {"help": "Preferred vehicle type"},
                },
                "car_delivery_time": {
                    "type": "string",
                    "format": "date-time",
                    "x-ui": {"help": "Requested pickup time (ISO 8601 format)"},
                },
                "booking_id": {"type": "string", "x-ui": {"help": "Booking ID for cancel or get_booking operations"}},
                "notes": {"type": "string", "x-ui": {"help": "Special instructions or notes"}},
            },
            "required": [],
    ...
    def get_output_schema(self) -> dict[str, Any] | None:
        ...
        vehicle_schema = {
            ...
            "properties": {
                ...
                "capacity": {
                    "type": "object",
                    "properties": {"passengers": {"type": "integer"}, "luggage": {"type": "integer"}},
                },
        ...
        return {
            ...
            "properties": {
                ...
                "booking": {**booking_schema, "description": "Booking confirmation details"},
    ...
    async def execute(self, params: dict[str, Any], context: Any, host: Any) -> ToolResult:
        ...
        if op == "check_availability":
            ...
            return ToolResult.ok(
                data={"available_vehicles": list(AVAILABLE_VEHICLES.values())},
        ...
        if op == "book":
            ...
            if not customer_id:
                return ToolResult.err("customer_id is required for book operation", code="missing_parameter")

            car_delivery_location = params.get("car_delivery_location")
            if not car_delivery_location:
                return ToolResult.err("car_delivery_location is required for book operation", code="missing_parameter")

            dropoff_location = params.get("dropoff_location")
            if not dropoff_location:
                return ToolResult.err("dropoff_location is required for book operation", code="missing_parameter")
            ...
            _booking_counter += 1
            booking_id = f"CAR-{datetime.now(UTC).strftime('%Y%m%d')}-{_booking_counter:03d}"
            ...
            if car_delivery_time_str:
                ...
                car_delivery_time = datetime.now(UTC) + timedelta(minutes=15)
            ...
            return ToolResult.ok(data={"booking": booking}, diagnostics=["Demo mode: using synthesized data"])

        if op == "cancel":
            booking_id = params.get("booking_id")
            if not booking_id:
                return ToolResult.err("booking_id is required for cancel operation", code="missing_parameter")
        ...
        if op == "get_booking":
            booking_id = params.get("booking_id")
            if not booking_id:
                return ToolResult.err("booking_id is required for get_booking operation", code="missing_parameter")
