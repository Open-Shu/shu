...
DEMO_PLAYER_PROFILES = {
    ...
    "CUST-5678": {
        ...
        "contact": {"email": "d.chen@example.com", "phone": "+1-555-0123", "preferred_contact": "text"},
        "hotel_preferences": {
        ...
        "dining_preferences": {
            ...
            "meal_times": {"breakfast": "07:00", "dinner": "20:00"},
        },
        "service_preferences": {
        ...
        "financial": {"lifetime_value": 125000.00, "ytd_value": 15000.00, "last_30_days_value": 8000.00},
        "visit_history": {
            "total_visits": 47,
        ...
        "special_dates": [{"type": "birthday", "date": "1978-06-15"}, {"type": "anniversary", "date": "2005-09-22"}],
        "notes": [
            {
                "date": "2025-12-20",
...
class DemoPlayerProfilesPlugin:
    ...
    def get_schema(self) -> dict[str, Any] | None:
        ...
        return {
            ...
            "properties": {
                ...
                "customer_id": {"type": "string", "x-ui": {"help": "Player ID for get operation"}},
                "customer_ids": {
                    "type": "array",
                    "items": {"type": "string"},
                    "x-ui": {"help": "Array of player IDs for get_by_players operation"},
                },
                "query": {"type": "string", "x-ui": {"help": "Search query for search operation"}},
                "include_analytics": {
                    "type": "boolean",
                    "default": True,
                    "x-ui": {"help": "Include analytics data in response"},
                },
                "include_notes": {
                    "type": "boolean",
                    "default": True,
                    "x-ui": {"help": "Include staff notes in response"},
                },
                "include_questions": {
                    "type": "boolean",
                    "default": True,
                    "x-ui": {"help": "Include pre-generated questions in response"},
                },
            },
            "required": [],
    ...
    def get_output_schema(self) -> dict[str, Any] | None:
        ...
        player_profile_schema = {
            ...
            "properties": {
                ...
                "member_since": {"type": "string", "format": "date", "description": "Membership start date"},
                "ban_date": {"type": "string", "format": "date", "description": "Date of ban (if applicable)"},
                "ban_reason": {"type": "string", "description": "Reason for ban (if applicable)"},
                "restriction_date": {
                    "type": "string",
                    "format": "date",
                    "description": "Date restriction was imposed (if applicable)",
                },
                "restriction_reason": {"type": "string", "description": "Reason for restriction (if applicable)"},
                "contact": {
                    "type": "object",
                    "properties": {
                        "email": {"type": "string"},
                ...
                "restriction_flags": {
                    "type": "object",
                    "properties": {
                        "banned": {"type": "boolean", "description": "Player is banned from property"},
                        "self_excluded": {"type": "boolean", "description": "Player has self-excluded"},
                        "alcohol_restricted": {"type": "boolean", "description": "Alcohol service is restricted"},
                        "credit_suspended": {"type": "boolean", "description": "Credit line is suspended"},
                        "watchlist": {
                            "type": "string",
                ...
                "ban_details": {"type": "object", "description": "Detailed information about ban (if applicable)"},
                "restriction_details": {
                    "type": "object",
                ...
                "preferences": {
                    ...
                    "properties": {
                        ...
                        "table_preference": {"type": "string", "description": "Preferred table atmosphere"},
                        "table_limits": {"type": "object", "description": "Preferred betting limits"},
                        "beverage": {"type": "string", "description": "Preferred beverage"},
                        ...
                        "room_preference": {"type": "string", "description": "Preferred room type"},
                    },
                ...
                "notes": {"type": "array", "items": {"type": "object"}, "description": "Staff notes and observations"},
                "security_notes": {
                    "type": "array",
                    "items": {"type": "object"},
        ...
        return {
            ...
            "type": "object",
            "properties": {
                "profile": {**player_profile_schema, "description": "Single player profile"},
                "profiles": {
                    "type": "array",
    ...
    async def execute(self, params: dict[str, Any], context: Any, host: Any) -> ToolResult:
        ...
        if op == "get":
            ...
            if not customer_id:
                return ToolResult.err("player_id is required for get operation", code="missing_parameter")

            # Get player profile from synthesized data
            profile = DEMO_PLAYER_PROFILES.get(customer_id)
            if profile is None:
                return ToolResult.err(f"Customer not found: {customer_id}", code="customer_not_found")

            filtered_profile = filter_profile(profile)

            return ToolResult.ok(data={"profile": filtered_profile}, diagnostics=["Demo mode: using synthesized data"])

        if op == "list":
            # Return all player profiles
            profiles = [filter_profile(profile) for profile in DEMO_PLAYER_PROFILES.values()]

            return ToolResult.ok(data={"profiles": profiles}, diagnostics=["Demo mode: using synthesized data"])

        if op == "search":
            query = params.get("query")
            if not query:
                return ToolResult.err("query is required for search operation", code="missing_parameter")
            ...
            return ToolResult.ok(
                data={"profiles": matching_profiles}, diagnostics=["Demo mode: using synthesized data"]
            )
