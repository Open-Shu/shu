...
class DemoSpaServicePlugin:
        ...
        return {
            ...
            "properties": {
                ...
                "customer_id": {
                    "type": "string",
                    "x-ui": {
                        "help": "Customer ID for the reservation"
                    }
                },
                "customer_name": {
                    "type": "string",
                    "x-ui": {
                        "help": "Customer name for the reservation"
                    }
                },
                "service_type": {
                    "type": "string",
                    ...
                    "x-ui": {
                        "help": "Type of spa service to reserve"
                    }
                },
                "appointment_time": {
                    "type": "string",
                    "format": "date-time",
                    "x-ui": {
                        "help": "Appointment time (ISO 8601 format)"
                    }
                },
                "party_size": {
                    "type": "integer",
                    "default": 1,
                    "x-ui": {
                        "help": "Number of people for the appointment"
                    }
                },
                "notes": {
                    "type": "string",
    ...
    def get_output_schema(self) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "properties": {
                "spa_availability": {
                    **availability_schema,
                    "description": "Spa availability information"
                },
                "reservation": {
                    **reservation_schema,
                    "description": "Spa reservation details"
                },
                "schedule": {
                    "type": "object",
                    "description": "Spa schedule information"
                },
                "message": {
                    "type": "string",
    ...
    async def execute(
        ...
        if op == "reserve":
            ...
            if not service_type:
                return ToolResult.err(
                    "service_type is required for reserve operation",
                    code="missing_parameter"
                )
            ...
            if appointment_time_str:
                ...
                now = datetime.now(timezone.utc).replace(tzinfo=timezone(timedelta(hours=4)))
                next_day = now + timedelta(days=1)
                appointment_time = next_day.replace(hour=10, minute=0, second=0, microsecond=0)
            
            # Generate reservation ID
            reservation_id = f"SPA-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}-{random.randint(100, 999)}"
            ...
            reservation = {
                ...
                "notes": notes,
                "status": "confirmed",
                "created_at": datetime.now(timezone.utc).replace(tzinfo=timezone(timedelta(hours=4))).isoformat()
            }
            ...
            return ToolResult.ok(
                data={"spa_availability": spa_availability},
                diagnostics=["Demo mode: using synthesized data"]
            )
            
        elif op == "get_schedule":
            # Return spa schedule
            now = datetime.now(timezone.utc).replace(tzinfo=timezone(timedelta(hours=4)))
            next_day = now + timedelta(days=1)
            base_time = next_day.replace(hour=10, minute=0, second=0, microsecond=0)
