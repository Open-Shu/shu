        ...
        try:
            ...
            for kb in kbs:
                if self.is_test_data(kb.get("name", "")):
                    self.would_delete["knowledge_bases"].append({"id": kb["id"], "name": kb["name"]})
                    ...
                    for source in sources:
                        if self.is_test_data(source.get("name", "")):
                            self.would_delete["sources"].append(
                                {"id": source["id"], "name": source["name"], "kb_name": kb["name"]}
                            )
                except Exception:
                    continue
        except Exception as e:
        ...
        try:
            ...
            for provider in providers:
                if self.is_test_data(provider.get("name", "")):
                    self.would_delete["llm_providers"].append({"id": provider["id"], "name": provider["name"]})
        except Exception as e:
            ...
            while True:
                ...
                for prompt in prompts:
                    if self.is_test_data(prompt.get("name", "")):
                        self.would_delete["prompts"].append({"id": prompt["id"], "name": prompt["name"]})
