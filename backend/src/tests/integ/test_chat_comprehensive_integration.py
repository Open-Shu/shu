...
from shu.services.providers.adapter_base import ProviderContentDeltaEventResult, ProviderFinalEventResult
from integ.helpers.api_helpers import process_streaming_result
from shu.llm.client import ProviderStreamEvent
...
async def test_llm_integration_with_error_handling(client, db, auth_headers):
    ...
    assert "Invalid API key or authentication failed" in content, f"Expected error message, got: {content}"
...
async def test_streaming_functionality(client, db, auth_headers):
    ...
    with patch('shu.llm.service.LLMService.get_client') as mock_client:
        ...
        print(f"Streaming response: {response.status_code} - Headers: {dict(response.headers)} - Content: {response.text}")
...
async def test_authentication_and_permissions(client, db, auth_headers):
    ...
    own_conversation_response = await client.get(
        f"/api/v1/chat/conversations/{conversation_id}",
        headers=auth_headers
    )
...
async def test_edge_cases_and_validation(client, db, auth_headers):
    ...
    if response.status_code in [400, 422]:
        ...
        assert any(keyword in error_message for keyword in ["empty", "required", "invalid"]), f"Expected validation error, got: {error_message}"

    # Test 2: Very long message (simplified - no complex mocking)
    long_message_data = {
        "message": "This is a test of a reasonably long message to verify the system can handle longer inputs without issues. " * 10,  # Reasonable length
        "rag_rewrite_mode": "no_rag",
    }
    ...
    assert long_response.status_code in [200, 422, 413, 500], "Should handle long messages gracefully"
    ...
    assert request.json()["detail"][0]["msg"] == "Extra inputs are not permitted", "Should reject additional conversation parameters"
