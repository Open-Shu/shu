"""
Integration test: calendar_events plugin executed via Plugin Feeds endpoints.
- Creates a KB
- Creates a feed for calendar_events using domain delegation or configured OAuth
...
async def _ensure_tool_enabled(client, db, auth_headers, name: str = "calendar_events"):
...
async def test_calendar_feed_run_now_domain_delegate(client, db, auth_headers):
    ...
    try:
        # Create a KB
        kb_payload = {
            "name": f"Calendar Feed KB {uuid.uuid4().hex[:6]}",
            "description": "KB for calendar feed execution",
            "sync_enabled": True,
        }
        ...
        feed_body: dict[str, Any] = {
            "name": f"Calendar Feed {uuid.uuid4().hex[:6]}",
        ...
        # Expect keys from calendar ingest (delta-friendly)
        assert "next_sync_token" in data or "count" in data, data
    finally:
...
if __name__ == "__main__":
    ...
    asyncio.run(run_integration_tests([test_calendar_feed_run_now_domain_delegate], enable_file_logging=True))
