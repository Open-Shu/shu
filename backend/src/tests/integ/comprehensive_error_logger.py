...
class ComprehensiveErrorLogger:
    ...
    FUNCTION_ERROR_PATTERNS = {
        "authentication_errors": [
            r".*unauthorized.*", r".*auth.*invalid.*", r".*missing.*auth.*",
            r".*invalid.*token.*", r".*malformed.*auth.*"
        ],
        "validation_errors": [
            r".*invalid.*data.*", r".*validation.*", r".*malformed.*",
            r".*empty.*data.*", r".*bad.*request.*"
        ],
        "not_found_errors": [
            r".*not.*found.*", r".*invalid.*id.*", r".*nonexistent.*",
            r".*fake.*id.*", r".*missing.*resource.*"
        ],
        "duplicate_errors": [
            r".*duplicate.*", r".*already.*exists.*", r".*same.*name.*"
        ],
        "llm_errors": [
            r".*send.*message.*", r".*llm.*", r".*chat.*completion.*",
            r".*streaming.*", r".*model.*discovery.*"
        ],
        "sync_errors": [
            r".*sync.*invalid.*", r".*sync.*error.*", r".*filesystem.*sync.*",
            r".*background.*sync.*"
        ],
        "database_errors": [
            r".*stale.*data.*", r".*rollback.*", r".*connection.*pool.*"
        ],
        "oauth_errors": [
            r".*oauth.*", r".*gmail.*", r".*credentials.*"
        ],
        "validation_pydantic_errors": [
            r".*pydantic.*", r".*conversation.*response.*", r".*validation.*error.*"
        ]
    }
    ...
    def wrap_test_execution(self, test_function_name: str):
        ...
        logger.info(f"=== EXPECTED TEST OUTPUT: Starting {test_function_name} with expected error types: {', '.join(error_types)} ===")
        ...
        try:
            ...
            logger.info(f"=== EXPECTED TEST OUTPUT: Completed {test_function_name} - any errors above were expected ===")
