...
async def test_detect_references_section_variations(client, db, auth_headers):
    ...
    for response in test_responses:
        ...
        assert len(analysis["reference_section_indicators"]) > 0, f"Should detect reference section in: {response[:50]}..."
...
async def test_detect_partial_title_matches(client, db, auth_headers):
    ...
    mock_sources = [
        {"document_title": "Machine Learning Applications Research Document.pdf", "source_url": "http://example.com/long"}
    ]
    ...
    for response in test_responses:
        ...
        assert analysis["has_source_citations"] or len(analysis["cited_sources"]) > 0, f"Should detect partial match in: {response[:50]}..."
...
async def test_detect_filename_mentions(client, db, auth_headers):
    ...
    for response in test_responses:
        analysis = analyze_response_references(response, mock_sources)
        assert analysis["has_source_citations"] or len(analysis["cited_sources"]) > 0, f"Should detect filename mention in: {response[:50]}..."
...
async def test_partial_citations_adds_missing(client, db, auth_headers):
    ...
    response = "Based on Cited Source 1 and Cited Source 2, we can conclude that the analysis shows significant results."
