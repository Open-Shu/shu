...
async def test_update_conversation_with_model_config(client, db, auth_headers):
    ...
    # Update conversation
    update_data = {"title": "Updated Test Conversation with Model Config"}
...
async def test_delete_conversation_with_model_config(client, db, auth_headers):
    ...
    # Delete conversation
    response = await client.delete(f"/api/v1/chat/conversations/{conversation_id}", headers=auth_headers)
    assert response.status_code == 204

    # Verify conversation is marked as inactive in database
    result = await db.execute(text("SELECT is_active FROM conversations WHERE id = :id"), {"id": conversation_id})
...
async def test_add_message_to_conversation(client, db, auth_headers):
    ...
    result = await db.execute(text("SELECT role, content FROM messages WHERE id = :id"), {"id": message["id"]})
    db_row = result.fetchone()
    assert db_row is not None
...
async def test_get_conversation_messages(client, db, auth_headers):
    ...
    # Get messages
    response = await client.get(f"/api/v1/chat/conversations/{conversation_id}/messages", headers=auth_headers)
    assert response.status_code == 200
...
async def test_create_conversation_with_model_config(client, db, auth_headers):
    ...
    ), f"Expected 201, got {provider_response.status_code}: {provider_response.text}"
...
async def test_send_message_with_model_config(client, db, auth_headers):
    ...
    logger.info(
        "=== EXPECTED TEST OUTPUT: Testing message sending with LLM provider (authentication errors expected) ==="
    )
    ...
    with expect_llm_errors():
        ...
        assert (
            message_response.status_code != 404
        ), f"Got 404 - this suggests endpoint construction bug: {message_response.text}"
        ...
        if message_response.status_code >= 400:
            ...
            ), f"Expected error details in response: {response_data}"
...
async def test_send_message_with_azure_openai_config(client, db, auth_headers):
    ...
    assert (
        message_response.status_code != 404
    ), f"Got 404 - this suggests Azure endpoint construction bug: {message_response.text}"
    ...
    if message_response.status_code >= 400:
        ...
        assert (
            "error" in response_data or "detail" in response_data
        ), f"Expected error details in Azure response: {response_data}"
    if hasattr(message_response, "aclose"):
        await message_response.aclose()
    else:
        message_response.close()
...
async def test_send_message_with_rag_enabled(client, db, auth_headers):
    ...
    # Create conversation
    conversation_data = {"title": "Test RAG Chat", "model_configuration_id": model_config_id}
    ...
    for mode_value, prompt in modes_to_test:
        send_data = {"message": prompt, "rag_rewrite_mode": mode_value}
        ...
        try:
            ...
            if diagnostics:
                assert (
                    diagnostics.get("mode") == mode_value
                ), f"Expected diagnostics mode {mode_value}, got {diagnostics.get('mode')}"
...
async def test_complex_relationship_loading(client, db, auth_headers):
    ...
    assert (
        retrieved_conversation["model_configuration"]["id"] == model_config_id
    ), "model_configuration ID mismatch on retrieval"
    ...
    message_data = {"role": "user", "content": "Test message for relationship validation"}
    ...
    print("✅ Complex relationship loading test passed - all SQLAlchemy relationships working correctly")
...
async def test_complex_relationship_loading(client, db, auth_headers):
    ...
    assert (
        "llm_provider_id" in model_config
    ), f"llm_provider_id missing from model_configuration response. Available fields: {list(model_config.keys())}"
    assert model_config["llm_provider_id"] == provider_id, "Provider ID mismatch in model_configuration"
