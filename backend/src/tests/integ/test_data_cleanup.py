...
class TestDataCleaner:
                ...
                if self.is_test_data(name):
                    try:
                        del_resp = await self.client.delete(
                            f"/api/v1/plugins/admin/feeds/{sched['id']}", headers=self.auth_headers
                        )
        ...
        try:
            ...
            while True:
                response = await self.client.get(
                    f"/api/v1/prompts/?limit={limit}&offset={offset}", headers=self.auth_headers
                )
                    ...
                    if is_test:
                        try:
                            delete_response = await self.client.delete(
                                f"/api/v1/prompts/{prompt['id']}", headers=self.auth_headers
                            )
                            if delete_response.status_code in [200, 204]:
                                ...
                                logger.warning(
                                    f"Failed to delete prompt {prompt['name']}: {delete_response.status_code}"
                                )
                        except Exception as e:
                    ...
                    if self.is_test_data(prompt.get("name", "")):
                        try:
                            ...
                            if assignments_response.status_code == 200:
                                ...
                                assignments = (
                                    assignments_data.get("items", assignments_data)
                                    if isinstance(assignments_data, dict)
                                    else assignments_data
                                )
                                ...
                                for assignment in assignments:
                                    try:
                                        ...
                                        if delete_response.status_code in [200, 204]:
                                            ...
                                            logger.info(
                                                f"🗑️  Cleaned up prompt assignment: {prompt['name']} -> {assignment['entity_id']}"
                                            )
                                    except Exception as e:
                                        error_msg = (
                                            f"Error deleting prompt assignment {assignment.get('entity_id')}: {e}"
                                        )
                                        logger.error(error_msg)
                                        self.cleanup_stats["errors"].append(error_msg)
                        except Exception as e:
    ...
    async def _cleanup_knowledge_bases(self):
        ...
        try:
            response = await self.client.get("/api/v1/knowledge-bases", headers=self.auth_headers)
            if response.status_code != 200:
            ...
            for kb in kbs:
                if self.is_test_data(kb.get("name", "")):
                    try:
                        delete_response = await self.client.delete(
                            f"/api/v1/knowledge-bases/{kb['id']}", headers=self.auth_headers
                        )
                        if delete_response.status_code in [200, 204]:
            ...
            for kb in kbs:
                try:
                    ...
                    sources_response = await self.client.get(
                        f"/api/v1/knowledge-bases/{kb['id']}/sources", headers=self.auth_headers
                    )
    ...
    async def _cleanup_sync_jobs(self):
        ...
        try:
            ...
            response = await self.client.get("/api/v1/sync/history", headers=self.auth_headers)
                ...
                if self.is_test_data(job_name) or self.is_test_data(kb_name):
                    try:
                        ...
                        if kb_id and job_id:
                            ...
                            # Now delete the job
                            delete_response = await self.client.delete(
                                f"/api/v1/sync/{kb_id}/jobs/{job_id}", headers=self.auth_headers
                            )
                            if delete_response.status_code in [200, 204]:
    ...
    async def _cleanup_llm_providers(self):
        ...
        try:
            response = await self.client.get("/api/v1/llm/providers", headers=self.auth_headers)
            if response.status_code != 200:
            ...
            for provider in providers:
                if self.is_test_data(provider.get("name", "")):
                    try:
                        delete_response = await self.client.delete(
                            f"/api/v1/llm/providers/{provider['id']}", headers=self.auth_headers
                        )
                        if delete_response.status_code in [200, 204]:
                            ...
                            logger.warning(
                                f"Failed to delete provider {provider['name']}: {delete_response.status_code}"
                            )
                    except Exception as e:
    ...
    async def _cleanup_model_configurations(self):
        ...
        try:
            response = await self.client.get("/api/v1/model-configurations", headers=self.auth_headers)
            if response.status_code != 200:
            ...
            for config in configurations:
                if self.is_test_data(config.get("name", "")):
                    try:
                        ...
                        if delete_response.status_code in [200, 204]:
                            self.cleanup_stats["model_configurations"] = (
                                self.cleanup_stats.get("model_configurations", 0) + 1
                            )
                            logger.info(f"🗑️  Cleaned up model configuration: {config['name']}")
                        else:
                            logger.warning(
                                f"Failed to delete model configuration {config['name']}: {delete_response.status_code}"
                            )
                    except Exception as e:
    ...
    async def _cleanup_conversations(self):
        ...
        try:
            response = await self.client.get("/api/v1/chat/conversations", headers=self.auth_headers)
        ...
        try:
            ...
            for user in users:
                ...
                if self.is_test_data(user_email) or self.is_test_data(user_name):
                    ...
                    try:
                        delete_response = await self.client.delete(
                            f"/api/v1/auth/users/{user['user_id']}", headers=self.auth_headers
                        )
                        if delete_response.status_code in [200, 204]:
