...
async def test_stop_word_consistency_across_search_types(client, db, auth_headers):
    ...
    for query in stop_word_queries:
        ...
        assert similarity_response.status_code in [200, 404], f"Similarity search failed for '{query}'"
        ...
        if keyword_response.status_code == 200:
            ...
            assert len(keyword_data.get("results", [])) == 0, f"Keyword search should return no results for stop word '{query}'"
...
async def test_mixed_query_consistency(client, db, auth_headers):
    ...
    for query in mixed_queries:
        ...
        # Both should succeed
        assert keyword_response.status_code in [200, 404], f"Keyword search failed for '{query}': {keyword_response.status_code} - {keyword_response.text}"
        assert hybrid_response.status_code in [200, 404], f"Hybrid search failed for '{query}': {hybrid_response.status_code} - {hybrid_response.text}"
        ...
        if keyword_response.status_code == 200 and hybrid_response.status_code == 200:
            ...
            assert keyword_data["query_type"] in ["keyword", "similarity"], f"Expected 'keyword' or 'similarity', got: {keyword_data.get('query_type')}"
            assert hybrid_data["query_type"] in ["hybrid", "similarity"], f"Expected 'hybrid' or 'similarity', got: {hybrid_data.get('query_type')}"
...
async def test_reference_handling_across_search_types(client, db, auth_headers):
    ...
    rag_config_response = await client.put(f"/api/v1/knowledge-bases/{kb_id}/rag-config",
                                          json={
...
async def test_empty_or_null_responses(client, db, auth_headers):
    ...
    for response in empty_responses:
        ...
        assert isinstance(analysis, dict), f"Should handle empty response: {repr(response)}"
