...
async def test_create_feature_item_success(client, db, auth_headers):
    ...
    response = await client.post("/api/v1/your-feature/items", json=SAMPLE_TEST_DATA, headers=auth_headers)
...
async def test_get_feature_item_by_id(client, db, auth_headers):
    ...
    create_response = await client.post("/api/v1/your-feature/items", json=SAMPLE_TEST_DATA, headers=auth_headers)
...
async def test_update_feature_item(client, db, auth_headers):
    ...
    response = await client.put(f"/api/v1/your-feature/items/{item_id}", json=update_data, headers=auth_headers)
    assert response.status_code == 200
    ...
    # Verify in database
    result = await db.execute(text("SELECT name, description FROM your_feature_table WHERE id = :id"), {"id": item_id})
    db_row = result.fetchone()
...
async def test_delete_feature_item(client, db, auth_headers):
    ...
    result = await db.execute(text("SELECT COUNT(*) FROM your_feature_table WHERE id = :id"), {"id": item_id})
    count = result.scalar()
    assert count == 0
...
async def test_invalid_data_validation(client, db, auth_headers):
    ...
    response = await client.post("/api/v1/your-feature/items", json=invalid_data, headers=auth_headers)
