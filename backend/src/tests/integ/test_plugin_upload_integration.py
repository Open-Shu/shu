...
from shu.core.config import get_settings_instance, Settings
...
async def test_upload_and_execute_plugin(client, db, auth_headers):
    ...
    # Enable plugin
    resp4 = await client.patch('/api/v1/plugins/admin/my_test_plugin/enable', json={'enabled': True}, headers=auth_headers)
    assert resp4.status_code == 200

    # Execute echo op
    resp5 = await client.post('/api/v1/plugins/my_test_plugin/execute', json={'params': {'op': 'echo', 'foo': 'bar'}}, headers=auth_headers)
