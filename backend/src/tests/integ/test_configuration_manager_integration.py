...
from shu.core.config import ConfigurationManager, get_config_manager, get_settings_instance
...
class ConfigurationManagerIntegrationTest:
    ...
    def test_configuration_dictionaries(self):
        ...
        try:
            ...
            expected_keys = {
                "instructional_prompt", "include_references", "reference_format",
                "context_format", "prompt_template", "search_threshold", 
                "max_results", "chunk_overlap_ratio", "search_type"
            }
            assert set(rag_dict.keys()) == expected_keys, f"Missing keys in RAG config dict"
