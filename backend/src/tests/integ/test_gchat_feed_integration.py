"""
Integration test: gchat_digest plugin executed via Plugin Feeds endpoints.
- Creates a KB
- Creates a feed for gchat_digest using domain delegation or configured OAuth
...
async def _ensure_tool_enabled(client, db, auth_headers, name: str = "gchat_digest"):
...
async def test_gchat_feed_run_now_domain_delegate(client, db, auth_headers):
    ...
    try:
        # Create a KB
        kb_payload = {
            "name": f"GChat Feed KB {uuid.uuid4().hex[:6]}",
            "description": "KB for gchat feed execution",
            "sync_enabled": True,
        }
