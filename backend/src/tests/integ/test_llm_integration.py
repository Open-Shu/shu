...
async def test_get_provider_by_id(client, db, auth_headers):
    ...
    create_response = await client.post("/api/v1/llm/providers", json=VALID_PROVIDER_DATA, headers=auth_headers)
...
async def test_delete_provider(client, db, auth_headers):
    ...
    result = await db.execute(text("SELECT COUNT(*) FROM llm_providers WHERE id = :id"), {"id": provider_id})
    count = result.scalar()
    assert count == 0
...
async def test_create_provider_duplicate_name(client, db, auth_headers):
    ...
    response1 = await client.post("/api/v1/llm/providers", json=VALID_PROVIDER_DATA, headers=auth_headers)
...
async def test_api_key_security_workflow(client, db, auth_headers):
    ...
    result = await db.execute(text("SELECT api_key_encrypted FROM llm_providers WHERE id = :id"), {"id": provider_id})
...
async def test_provider_type_endpoints(client, db, auth_headers):
    ...
    response = await client.get("/api/v1/llm/provider-types", headers=auth_headers)
    assert response.status_code == 200
    assert response.json().get("data") is not None
    assert len(response.json().get("data")) > 0
    openai = next(filter(lambda x: x["key"] == "openai", response.json().get("data", [])), None)
    assert set(openai.keys()) == set(["key", "display_name", "provider_adapter_name", "is_active"])

    response = await client.get("/api/v1/llm/provider-types/openai", headers=auth_headers)
    assert response.status_code == 200
    assert response.json().get("data") is not None
