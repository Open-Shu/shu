...
class ReportGenerator:
    ...
    def _write_executive_summary(self, results: BenchmarkResults, path: Path) -> None:
        ...
        if refs:
            ...
            # Our systems (using our own measured scores)
            if our_bm25_ndcg > 0:
                all_rows.append(("**Shu BM25 (measured)**", our_bm25_ndcg, 0.0, "**lexical (ours)**"))
                all_rows.append(("**Shu chunk similarity**", baseline_ndcg, our_baseline_vs_bm25, "**dense (ours)**"))
                all_rows.append(("**Shu multi-surface fusion**", ms_ndcg, our_fusion_vs_bm25, "**multi-surface (ours)**"))
                all_rows.append(("**Shu best novel surface**", our_best_ndcg, our_best_vs_bm25, "**multi-surface (ours)**"))
