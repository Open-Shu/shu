...
def judge_candidates(candidates: list[CandidateDoc]) -> list[RelevanceJudgment]:
    """Judge relevance of each candidate document for its query.

    This function is designed to be called by an LLM (Claude) that reads
    each query-document pair and makes a relevance judgment. The LLM
    should be given the candidates and return judgments.

    Relevance scale:
        0 = Not relevant: Document does not help answer the query.
        1 = Partially relevant: Document contains tangentially related
            information that could inform but not directly answer the query.
        2 = Highly relevant: Document directly helps answer the query,
            containing specific information, evidence, or data that
            addresses what the query is asking.

    The judgment should consider cross-topic connections. For example,
    "Are avocados good for you?" should match a paper about fatty acid
    intake and prostate cancer if the paper identifies avocado as the
    principal source of the beneficial fatty acid.

    Args:
        candidates: List of CandidateDoc to judge.

    Returns:
        List of RelevanceJudgment, one per candidate.
    """
    # This is a placeholder — the actual judging happens externally
    # (by Claude reading the candidates and producing judgments).
    # See judge_batch() for the batch processing interface.
    raise NotImplementedError(
        "judge_candidates is called externally by an LLM. "
        "Use judge_batch() for programmatic batch processing, "
        "or generate judgments manually and load via load_judgments()."
    )


def judge_batch(candidates: list[CandidateDoc], batch_size: int = 10) -> list[dict]:
    """Prepare batches of candidates for LLM judgment.

    Formats candidates into prompts that can be presented to an LLM
    for relevance assessment. Each batch contains multiple query-document
    pairs to judge.
    ...
    Returns:
        List of batch dicts, each containing:
            - "batch_id": int
            - "candidates": list of {query_id, query_text, doc_id, doc_title, doc_text}
            - "prompt": formatted prompt string for LLM judgment
    """
    batches = []
    for i in range(0, len(candidates), batch_size):
        ...
        prompt_parts = [
            "Judge the relevance of each document to its query.\n",
            "For each pair, respond with a JSON object: "
            '{"query_id": "...", "doc_id": "...", "score": 0|1|2, "justification": "..."}\n',
            "Scoring:\n"
            "  0 = Not relevant: Document does not help answer the query.\n"
            "  1 = Partially relevant: Contains tangentially related information.\n"
            "  2 = Highly relevant: Directly helps answer the query with specific "
            "information, evidence, or data.\n\n"
            "Consider cross-topic connections. A paper about fatty acids and cancer "
            "IS relevant to 'Are avocados good for you?' if it identifies avocado "
            "as a source of the beneficial fatty acid.\n",
        ]

        for j, c in enumerate(batch_candidates):
            prompt_parts.append(f"\n--- Pair {j + 1} ---")
            prompt_parts.append(f"Query ID: {c.query_id}")
            prompt_parts.append(f"Query: {c.query_text}")
            prompt_parts.append(f"Doc ID: {c.doc_id}")
            prompt_parts.append(f"Title: {c.doc_title}")
            prompt_parts.append(f"Text: {c.doc_text[:2000]}")  # Truncate very long docs
            prompt_parts.append("")
    ...
    logger.info("Prepared %d batches of %d candidates each", len(batches), batch_size)
    return batches
