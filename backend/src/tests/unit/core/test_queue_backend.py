    ...
    **Validates: Requirements 11.3**
    ...
    async def test_job_serialization_round_trip(self, job: Job):
        ...
        **Validates: Requirements 11.3**
        ...
        assert (
            restored_job.queue_name == job.queue_name
        ), f"queue_name mismatch: {restored_job.queue_name} != {job.queue_name}"
        assert restored_job.payload == job.payload, f"payload mismatch: {restored_job.payload} != {job.payload}"
        assert (
            restored_job.created_at == job.created_at
        ), f"created_at mismatch: {restored_job.created_at} != {job.created_at}"
        assert restored_job.attempts == job.attempts, f"attempts mismatch: {restored_job.attempts} != {job.attempts}"
        assert (
            restored_job.max_attempts == job.max_attempts
        ), f"max_attempts mismatch: {restored_job.max_attempts} != {job.max_attempts}"
        assert (
            restored_job.visibility_timeout == job.visibility_timeout
        ), f"visibility_timeout mismatch: {restored_job.visibility_timeout} != {job.visibility_timeout}"
...
class TestExceptionHierarchy:
    ...
    def test_queue_error_with_details(self):
        ...
        error = QueueError("Something went wrong", details={"key": "value", "code": 123})
...
class TestInMemoryQueueBackendProtocol:
    ...
    def test_inmemory_queue_backend_implements_protocol(self, inmemory_queue_backend: InMemoryQueueBackend):
...
class TestRedisQueueBackendProtocol:
    ...
    def test_redis_queue_backend_implements_protocol(self, redis_queue_backend: RedisQueueBackend):
    ...
    **Validates: Requirements 10.1**
    ...
    async def test_enqueue_dequeue_round_trip(self, job: Job):
        ...
        **Validates: Requirements 10.1**
        ...
        assert (
            dequeued.attempts == original_attempts + 1
        ), f"attempts should be incremented by 1: {dequeued.attempts} != {original_attempts + 1}"

    @pytest.mark.asyncio
    async def test_enqueue_dequeue_preserves_all_fields(self, inmemory_queue_backend: InMemoryQueueBackend):
    ...
    @pytest.mark.asyncio
    async def test_dequeue_empty_queue_returns_none(self, inmemory_queue_backend: InMemoryQueueBackend):
        """Unit test: dequeue from empty queue returns None."""
        result = await inmemory_queue_backend.dequeue("empty_queue")
        assert result is None

    @pytest.mark.asyncio
    async def test_fifo_order_preserved(self, inmemory_queue_backend: InMemoryQueueBackend):
        """Unit test: jobs are dequeued in FIFO order."""
        jobs = [Job(queue_name="test", payload={"order": i}) for i in range(5)]
    ...
    **Validates: Requirements 3.3, 10.3**
    ...
    async def test_visibility_timeout_expiration(self, queue_name: str, payload: dict[str, Any]):
        ...
        **Validates: Requirements 3.3, 10.3**
        ...
        assert (
            redelivered.attempts == dequeued.attempts + 1
        ), f"Attempts should be incremented: {redelivered.attempts} != {dequeued.attempts + 1}"

    @pytest.mark.asyncio
    async def test_acknowledged_job_not_redelivered_after_timeout(self, inmemory_queue_backend: InMemoryQueueBackend):
    ...
    @pytest.mark.asyncio
    async def test_visibility_timeout_with_real_time(self, inmemory_queue_backend: InMemoryQueueBackend):
    ...
    **Validates: Requirements 3.2**
    ...
    async def test_concurrent_enqueue_no_jobs_lost(self, num_jobs: int):
        ...
        **Validates: Requirements 3.2**
        ...
        jobs = [Job(queue_name=queue_name, payload={"index": i}) for i in range(num_jobs)]
    ...
    async def test_concurrent_dequeue_no_duplicates(self, num_jobs: int):
        ...
        **Validates: Requirements 3.2**
        ...
        assert len(dequeued_jobs) == num_jobs, f"Expected {num_jobs} dequeued jobs, got {len(dequeued_jobs)}"
    ...
    @pytest.mark.asyncio
    async def test_concurrent_enqueue_and_dequeue(self, inmemory_queue_backend: InMemoryQueueBackend):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_acknowledge_operations(self, inmemory_queue_backend: InMemoryQueueBackend):
...
class TestProperty2EnqueueDequeueRoundTripParametrized:
    ...
    **Validates: Requirements 2.5, 10.1**
    ...
    @pytest.mark.asyncio
    async def test_enqueue_dequeue_preserves_all_fields_inmemory(self, inmemory_queue_backend: InMemoryQueueBackend):
    ...
    @pytest.mark.asyncio
    async def test_enqueue_dequeue_preserves_all_fields_redis(self, redis_queue_backend: RedisQueueBackend):
    ...
    @pytest.mark.asyncio
    async def test_dequeue_empty_queue_returns_none_inmemory(self, inmemory_queue_backend: InMemoryQueueBackend):
        """Unit test: dequeue from empty queue returns None (InMemory)."""
        result = await inmemory_queue_backend.dequeue("empty_queue_test")
        assert result is None

    @pytest.mark.asyncio
    async def test_dequeue_empty_queue_returns_none_redis(self, redis_queue_backend: RedisQueueBackend):
        """Unit test: dequeue from empty queue returns None (Redis)."""
        result = await redis_queue_backend.dequeue("empty_queue_test")
        assert result is None

    @pytest.mark.asyncio
    async def test_fifo_order_preserved_inmemory(self, inmemory_queue_backend: InMemoryQueueBackend):
        """Unit test: jobs are dequeued in FIFO order (InMemory)."""
        queue_name = "fifo_test_queue"
        jobs = [Job(queue_name=queue_name, payload={"order": i}) for i in range(5)]
    ...
    @pytest.mark.asyncio
    async def test_fifo_order_preserved_redis(self, redis_queue_backend: RedisQueueBackend):
...
class TestProperty3VisibilityTimeoutExpirationParametrized:
    ...
    **Validates: Requirements 2.5, 3.3, 10.3**
    ...
    @pytest.mark.asyncio
    async def test_visibility_timeout_with_real_time_inmemory(self, inmemory_queue_backend: InMemoryQueueBackend):
    ...
    @pytest.mark.asyncio
    async def test_visibility_timeout_with_real_time_redis(self, redis_queue_backend: RedisQueueBackend):
    ...
    @pytest.mark.asyncio
    async def test_acknowledged_job_not_redelivered_inmemory(self, inmemory_queue_backend: InMemoryQueueBackend):
    ...
    @pytest.mark.asyncio
    async def test_acknowledged_job_not_redelivered_redis(self, redis_queue_backend: RedisQueueBackend):
    ...
    **Validates: Requirements 10.1**
    ...
    async def test_acknowledged_jobs_not_redelivered_inmemory(self, job: Job):
        ...
        **Validates: Requirements 10.1**
        ...
        with backend._lock:
            # Job should not be in processing anymore
            assert (
                job.id not in backend._processing[job.queue_name]
        ...
        assert redelivered is None, "Acknowledged job should NOT be redelivered"
    ...
    async def test_acknowledged_jobs_not_redelivered_redis(self, job: Job):
        ...
        **Validates: Requirements 10.1**
        ...
        assert job.id not in members, "Acknowledged job should be removed from processing set"
    ...
    @pytest.mark.asyncio
    async def test_double_acknowledge_returns_false_inmemory(self, inmemory_queue_backend: InMemoryQueueBackend):
    ...
    @pytest.mark.asyncio
    async def test_double_acknowledge_returns_false_redis(self, redis_queue_backend: RedisQueueBackend):
    ...
    **Validates: Requirements 10.1**
    ...
    async def test_rejected_jobs_with_requeue_redelivered_inmemory(
        ...
        **Validates: Requirements 10.1**
        ...
        redelivered = await backend.dequeue(queue_name)
        assert redelivered is not None, "Rejected job with requeue=True should be redelivered"
        assert redelivered.id == job.id, "Redelivered job should have same ID"
        assert (
    ...
    @pytest.mark.asyncio
    async def test_rejected_job_without_requeue_not_redelivered_redis(self, redis_queue_backend: RedisQueueBackend):
    ...
    @pytest.mark.asyncio
    async def test_rejected_job_at_max_attempts_not_redelivered_redis(self, redis_queue_backend: RedisQueueBackend):
    ...
    @pytest.mark.asyncio
    async def test_multiple_reject_requeue_cycles_inmemory(self, inmemory_queue_backend: InMemoryQueueBackend):
    ...
    @pytest.mark.asyncio
    async def test_multiple_reject_requeue_cycles_redis(self, redis_queue_backend: RedisQueueBackend):
    ...
    async def test_factory_returns_redis_backend_when_redis_available(self):
        ...
        try:
            with patch("shu.core.config.get_settings_instance", return_value=mock_settings):
                with patch(
                    "shu.core.queue_backend._get_shared_redis_client",
                    mock_get_shared_redis_client_success,
                ):
                    backend = await get_queue_backend()
