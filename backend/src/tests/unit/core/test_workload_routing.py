    ...
    async def test_workload_type_routing_correctness(
        ...
        **Validates: Requirements 5.2, 5.3, 5.4**
        ...
        assert job.queue_name == queue_name, (
            f"Job queue_name should match: {job.queue_name} != {queue_name}"
        )
        assert job.payload == payload, (
            f"Job payload should match: {job.payload} != {payload}"
        )
        ...
        assert dequeued.id == job.id, (
            f"Dequeued job ID should match: {dequeued.id} != {job.id}"
        )
        assert dequeued.payload == payload, (
            f"Dequeued job payload should match: {dequeued.payload} != {payload}"
        )
        ...
        **Validates: Requirements 5.2, 5.3**
        ...
        if workload_type_1 == workload_type_2:
            ...
            assert queue_name_1 == queue_name_2, (
                f"Same WorkloadType should map to same queue: "
                f"{queue_name_1} != {queue_name_2}"
            )
        else:
