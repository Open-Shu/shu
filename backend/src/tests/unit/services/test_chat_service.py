...
class TestChatServiceFavoriteOwnership:
    ...
    @pytest.mark.asyncio
    async def test_owner_can_update_favorite_status(self, conv_data: dict[str, Any], new_favorite_status: bool) -> None:
        """
        ...
        **Validates: Requirements 2.8**
        ...
        result = await chat_service.update_conversation(
            conversation_id=conv_data["id"], is_favorite=new_favorite_status
        )
    ...
    async def test_favorite_status_persists_after_update(
        self, conv_data: dict[str, Any], new_favorite_status: bool
    ) -> None:
        """
        ...
        **Validates: Requirements 2.8, 2.1, 2.6**
        ...
        assert mock_conversation.is_favorite == new_favorite_status, (
            f"Expected is_favorite={new_favorite_status} after update, " f"but got {mock_conversation.is_favorite}"
        )
    ...
    async def test_nonexistent_conversation_raises_error(
        ...
        **Validates: Requirements 2.8, 2.9**
        ...
        with pytest.raises(ConversationNotFoundError) as exc_info:
            await chat_service.update_conversation(conversation_id=conv_data["id"], is_favorite=new_favorite_status)
    ...
    async def test_favorite_update_with_other_fields(
        self, conv_data: dict[str, Any], new_favorite_status: bool, new_title: str
    ) -> None:
        """
        ...
        **Validates: Requirements 2.8, 2.6**
        ...
        result = await chat_service.update_conversation(
            conversation_id=conv_data["id"], title=new_title, is_favorite=new_favorite_status
        )

        # Property: Both fields should be updated
        assert (
            mock_conversation.is_favorite == new_favorite_status
        ), f"Expected is_favorite={new_favorite_status}, but got {mock_conversation.is_favorite}"
        assert (
            mock_conversation.title == new_title
    ...
    @settings(max_examples=100)
    @pytest.mark.asyncio
    async def test_favorite_toggle_idempotency(self, conv_data: dict[str, Any]) -> None:
        """
        ...
        **Validates: Requirements 2.8, 2.6**
        ...
        result = await chat_service.update_conversation(
            conversation_id=conv_data["id"], is_favorite=current_favorite_status
        )
        ...
        assert mock_conversation.is_favorite == current_favorite_status, (
            f"Expected is_favorite to remain {current_favorite_status}, " f"but got {mock_conversation.is_favorite}"
        )
    ...
    **Validates: Requirements 2.3, 2.4, 2.5**
    ...
    async def test_favorited_conversations_appear_first(self) -> None:
        ...
        **Validates: Requirements 2.3**
        ...
        # Get conversations
        conversations = await chat_service.get_user_conversations(user_id=user_id, limit=50, offset=0)
    ...
    async def test_mixed_favorite_and_non_favorite_ordering(self) -> None:
        ...
        **Validates: Requirements 2.3, 2.4, 2.5**
        ...
        sorted_conversations = sorted(mock_conversations, key=lambda c: (not c.is_favorite, -c.updated_at.timestamp()))
...
class TestChatServiceExperienceIntegration:
    ...
    **Validates: Requirements 1.2, 1.4, 2.4, 5.3**
    ...
    @settings(max_examples=100)
    @pytest.mark.asyncio
    async def test_conversation_creation_with_prefilled_message(self, run_data: dict[str, Any]) -> None:
        """
        ...
        **Validates: Requirements 1.2, 1.4, 2.4, 5.3**
        ...
        result = await chat_service.create_conversation_from_experience_run(
            run_id=run_data["run_id"], user_id=run_data["user_id"], title_override=None
        )
        ...
        assert len(conversations) == 1, f"Expected exactly 1 conversation to be created, but got {len(conversations)}"
        assert len(messages) == 1, f"Expected exactly 1 message to be created, but got {len(messages)}"

        conversation = conversations[0]
        message = messages[0]

        # Property 1c: Message should be an assistant message
        assert message.role == "assistant", f"Expected message role to be 'assistant', but got '{message.role}'"
        ...
        assert message.conversation_id == conversation.id, "Message conversation_id should match conversation id"
        ...
        assert (
            message.message_metadata.get("source") == "experience_result"
        ), f"Expected message metadata source='experience_result', but got '{message.message_metadata.get('source')}'"
        assert (
            message.message_metadata.get("experience_run_id") == run_data["run_id"]
    ...
    @settings(max_examples=100)
    @pytest.mark.asyncio
    async def test_conversation_title_override(self, run_data: dict[str, Any], custom_title: str) -> None:
        """
        ...
        **Validates: Requirements 1.3**
        ...
        await chat_service.create_conversation_from_experience_run(
            run_id=run_data["run_id"], user_id=run_data["user_id"], title_override=custom_title
        )
        ...
        conversation = conversations[0]
        assert conversation.title == custom_title, f"Expected title='{custom_title}', but got '{conversation.title}'"

    @given(experience_run_data())
    @settings(max_examples=100)
    @pytest.mark.asyncio
    async def test_conversation_uses_default_title(self, run_data: dict[str, Any]) -> None:
        """
        ...
        **Validates: Requirements 1.3**
        ...
        conversation = conversations[0]
        assert (
            conversation.title == run_data["experience_name"]
        ), f"Expected title='{run_data['experience_name']}', but got '{conversation.title}'"

    @given(experience_run_data())
    @settings(max_examples=100)
    @pytest.mark.asyncio
    async def test_model_configuration_priority_cascade(self, run_data: dict[str, Any]) -> None:
        """
        ...
        **Validates: Requirements 3.1, 3.2, 3.3, 3.4**
        ...
        expected_model_config_id = run_data["model_configuration_id"] or run_data["experience_model_configuration_id"]
    ...
    @settings(max_examples=100)
    @pytest.mark.asyncio
    async def test_complete_metadata_preservation(self, run_data: dict[str, Any]) -> None:
        """
        ...
        **Validates: Requirements 1.7, 2.1, 2.2, 2.3, 5.5**
        ...
        for field, expected_value in required_fields.items():
            ...
            assert actual_value == expected_value, f"Expected meta.{field}='{expected_value}', but got '{actual_value}'"
    ...
    async def test_missing_model_configuration_raises_error(self) -> None:
        ...
        **Validates: Requirements 1.2** (conversations require model configurations)
        ...
        with pytest.raises(HTTPException) as exc_info:
            await chat_service.create_conversation_from_experience_run(
                run_id=run_id, user_id=user_id, title_override=None
            )
