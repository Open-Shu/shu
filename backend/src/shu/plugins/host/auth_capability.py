...
class AuthCapability(ImmutableCapabilityMixin):
    ...
    __slots__ = ("_plugin_name", "_user_id", "_http", "_settings", "_ctx", "_primary_emails")
    ...
    _primary_emails: Dict[str, Optional[str]]

    def __init__(self, *, plugin_name: str, user_id: str, http: Optional[HttpCapability] = None, context: Optional[Dict[str, Any]] = None, provider_primary_emails: Optional[Dict[str, Optional[str]]] = None):
        ...
        object.__setattr__(self, "_primary_emails", {str(k).lower(): (v if (v is None or isinstance(v, str)) else None) for k, v in (provider_primary_emails or {}).items()})

    def get_context_for(self, provider: str) -> Optional[Dict[str, Any]]:
        try:
            ctx = self._ctx or {}
    ...
    async def _post_form(self, url: str, data: Dict[str, str]) -> Dict[str, Any]:
        resp = await self._http.fetch("POST", url, data=data, headers={"Content-Type": "application/x-www-form-urlencoded"})
    ...
    async def user_google_token(self, *, required_scopes: Optional[List[str]] = None) -> Optional[str]:
        ...
        try:
            ...
            try:
                ...
                if req and not req.issubset(granted):
                    logger.warning("user_google_token: insufficient scopes; required=%s granted=%s", list(req), list(granted))
                    return None
            except Exception:
                pass
            ...
            if req_scopes_list:
                ...
                try:
                    logger.info("user_google_token: refresh with scope param", extra={"scopes": req_scopes_list})
                except Exception:
                    pass
                ...
                if req_scopes:
                    ...
                    if req_scopes and token_scopes and not req_scopes.issubset(token_scopes):
                        logger.warning(
                            ...
                            list(req_scopes), list(token_scopes),
                        )
                        return None
    ...
    async def jwt_bearer_assertion(
        ...
        *,
        ...
        scope_str = scopes if isinstance(scopes, str) else " ".join(scopes)
        claims: Dict[str, Any] = {"iss": issuer, "scope": scope_str, "aud": aud, "iat": now, "exp": exp}
        if subject:
            claims["sub"] = subject
        if extra_claims:
        ...
        logger.info("jwt_bearer_assertion: preparing assertion", extra={
            "token_url": token_url,
            "issuer": issuer,
            "subject": subject or "",
            "audience": aud,
        ...
        cache_key = ("jwt_bearer", token_url, issuer, subject or "", scope_str, str(hash(private_key_pem)))
        cached = _TOKEN_CACHE.get(cache_key)
        if cached and cached[0] - 60 > now:
            return cached[1]

        body = await self._post_form(token_url, {
            "grant_type": "urn:ietf:params:oauth:grant-type:jwt-bearer",
            "assertion": assertion,
        })
        token = body.get("access_token")
        if not token:
            logger.error("jwt_bearer_assertion: token missing in response", extra={
                "token_url": token_url,
                "issuer": issuer,
                "subject": subject or "",
    ...
    async def google_service_account_token(self, *, scopes: List[str], subject: Optional[str] = None) -> str:
        import json, os
        ...
        if self._settings.google_service_account_json:
            ...
            try:
                ...
                raise RuntimeError("Invalid GOOGLE_SERVICE_ACCOUNT_JSON content or path; set GOOGLE_SERVICE_ACCOUNT_JSON to a file path or inline JSON") from e
            ...
            raise RuntimeError("Service account credentials not configured (set GOOGLE_SERVICE_ACCOUNT_JSON to a file path or inline JSON)")
    ...
    async def google_domain_delegation_check(self, *, scopes: List[str], subject: str) -> Dict[str, Any]:
        ...
        if not raw:
            return {"ready": False, "status": 0, "error": {"message": "Service account not configured (GOOGLE_SERVICE_ACCOUNT_JSON)"}}
        try:
            ...
            return {"ready": False, "status": 0, "error": {"message": f"Failed to load service account JSON: {e}"}}
        ...
        if not (client_email and private_key):
            return {"ready": False, "status": 0, "client_id": client_id, "issuer": client_email, "error": {"message": "Service account JSON missing client_email or private_key"}}
        if "\\n" in private_key:
            private_key = private_key.replace("\\n", "\n")
        try:
            from jose import jwt  # type: ignore
        except Exception as e:
            return {"ready": False, "status": 0, "client_id": client_id, "issuer": client_email, "error": {"message": f"python-jose not available: {e}"}}
        now = int(time.time())
        exp = now + 3600
        aud = token_uri
        scope_str = " ".join(scopes)
        claims: Dict[str, Any] = {"iss": client_email, "scope": scope_str, "aud": aud, "iat": now, "exp": exp, "sub": subject}
        headers = {"alg": "RS256", "typ": "JWT"}
        assertion = jwt.encode(claims, private_key, algorithm="RS256", headers=headers)
        try:
            resp = await self._http.fetch("POST", token_uri, data={
                "grant_type": "urn:ietf:params:oauth:grant-type:jwt-bearer",
                "assertion": assertion,
            ...
            logger.warning("Google domain delegation check HTTP error", extra={"status_code": e.status_code, "url": e.url})
            status = int(getattr(e, "status_code", 0))
            body = getattr(e, "body", None)
        ok = (status == 200) and isinstance(body, dict) and bool(body.get("access_token"))
        if ok:
            return {"ready": True, "status": status, "client_id": client_id, "issuer": client_email, "scopes": scopes}
        err_msg = None
    ...
    async def provider_user_token(self, provider: str, *, required_scopes: Optional[List[str]] = None) -> Optional[str]:
        prov = (provider or "").lower().strip()
        adapter = get_auth_adapter(prov, self)
        return await adapter.user_token(required_scopes=required_scopes)

    async def provider_service_account_token(self, provider: str, *, scopes: List[str], subject: Optional[str] = None) -> str:
        prov = (provider or "").lower().strip()
        adapter = get_auth_adapter(prov, self)
    ...
    async def resolve_token_and_target(self, provider: str, *, scopes: Optional[List[str]] = None) -> Tuple[Optional[str], Optional[str]]:
        try:
