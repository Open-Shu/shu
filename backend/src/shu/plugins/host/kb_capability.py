...
from ...services.ingestion_service import (
    ingest_document as _host_ingest_document,
    ingest_email as _host_ingest_email,
    ingest_text as _host_ingest_text,
    ingest_thread as _host_ingest_thread,
)
from ...services.knowledge_object_service import (
    upsert_knowledge_object as _host_upsert_knowledge_object,
    delete_ko_by_external_id as _host_delete_ko_by_external_id,
    delete_kos_by_external_ids as _host_delete_kos_by_external_ids,
)
...
class KbCapability(ImmutableCapabilityMixin):
    ...
    __slots__ = ("_plugin_name", "_user_id", "_schedule_id", "_ocr_mode")
    ...
    def __init__(self, *, plugin_name: str, user_id: str, ocr_mode: Optional[str] = None, schedule_id: Optional[str] = None):
    ...
    async def upsert_knowledge_object(self, knowledge_base_id: str, ko: Union[Dict[str, Any], "KnowledgeObject"]) -> str:
        # Normalize KO
        ...
        try:
            ...
            logger.info("host.kb.upsert", extra={"plugin": self._plugin_name, "user_id": self._user_id, "kb": knowledge_base_id, "ko_id": ko_id})
            return ko_id
        finally:
            try:
                await db.close()
    ...
    async def delete_ko(self, *, external_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            out = await _host_delete_ko_by_external_id(db, kb_id=kb_id, external_id=external_id, plugin_name=self._plugin_name)
            try:
                logger.info("host.kb.delete_ko", extra={
                    "plugin": self._plugin_name, "user_id": self._user_id, "kb": kb_id, "schedule_id": self._schedule_id,
                    "external_id": external_id, "deleted": bool(out.get("deleted")), "ko_id": out.get("ko_id")
                })
            except Exception:
                pass
            return out
    ...
    async def delete_kos_batch(self, *, external_ids: list[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            out = await _host_delete_kos_by_external_ids(db, kb_id=kb_id, external_ids=ids, plugin_name=self._plugin_name)
            try:
                logger.info("host.kb.delete_kos_batch", extra={
                    "plugin": self._plugin_name, "user_id": self._user_id, "kb": kb_id, "schedule_id": self._schedule_id,
                    "deleted_count": out.get("deleted_count"), "failed": out.get("failed")
                })
            except Exception:
                pass
            return out
