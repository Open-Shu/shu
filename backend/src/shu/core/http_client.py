    ...
    async def get_client(self) -> httpx.AsyncClient:
        ...
        if self._client is None:
            ...
            self._client = httpx.AsyncClient(
                ...
                timeout=httpx.Timeout(connect=self._settings.llm_global_timeout, read=self._settings.llm_streaming_read_timeout, write=self._settings.llm_global_timeout, pool=self._settings.llm_global_timeout),
                follow_redirects=True,
                headers={
                    "User-Agent": f"Shu-RAG-Backend/{self._settings.version}"
                }
            )
        return self._client
