...
class RAGConfig(BaseModel):
    ...
    include_references: bool = Field(default=True, description="Include references section in response")
    ...
    reference_format: str = Field(default="markdown", description="Reference format: 'markdown' or 'text'")
    context_format: str = Field(default="detailed", description="Context format: 'detailed' or 'simple'")
    ...
    search_type: str = Field(default="hybrid", description="Search type: 'similarity', 'keyword', or 'hybrid'")

    # Title Search Configuration
    title_weighting_enabled: bool = Field(default=True, description="Enable title weighting in search results")
    title_weight_multiplier: float = Field(
        default=3.0, ge=1.0, le=10.0, description="Multiplier for title match scores (1.0-10.0)"
    )
    title_chunk_enabled: bool = Field(
        default=True, description="Create dedicated title chunks for better title matching"
    )
    max_chunks_per_document: int = Field(
        default=4, ge=1, le=10, description="Maximum number of chunks to return per document (1-10)"
    )
...
class RAGConfigResponse(BaseModel):
    ...
    # Full Document Escalation
    fetch_full_documents: bool = Field(
        False, description="When enabled, escalate to fetching full documents for top results"
    )
