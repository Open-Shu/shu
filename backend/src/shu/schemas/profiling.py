...
class ProfilingResult(BaseModel):
    ...
    document_profile: Optional[DocumentProfile] = Field(
        None, description="Document-level profile (may be None if failed)"
    )
    chunk_profiles: List[ChunkProfileResult] = Field(
        default_factory=list, description="Profiles for each chunk"
    )
    profiling_mode: ProfilingMode = Field(
        ..., description="Whether full-doc or chunk-aggregation was used"
    )
