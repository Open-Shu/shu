...
class EndpointDefUpdate(BaseModel):
    ...
    options: dict[str, Any] | None = Field(
        None, description="Endpoint-specific options (e.g., discovery paths, message paths)"
    )
...
class LLMProviderCreate(BaseModel):
    ...
    budget_limit_monthly: float | None = Field(None, description="Monthly budget limit in dollars")
    endpoints: dict[str, EndpointDefUpdate] | None = Field(
        None, description="Overrides for ProviderTypeDefinition.endpoints stored on provider"
    )
    provider_capabilities: dict[str, ProviderCapabilitiesUpdate] | None = Field(
        None, description="Overrides for ProviderTypeDefinition.endpoints stored on provider"
    )
...
class LLMProviderResponse(BaseModel):
    ...
    endpoints: dict[str, Any] | None = Field(None, description="Saved endpoint overrides on provider.config")
    provider_capabilities: dict[str, Any] | None = Field(
        None, description="Provider capability overrides on provider.config"
    )
    created_at: datetime
    updated_at: datetime
...
async def list_providers(current_user: User = Depends(require_admin), db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        return ShuResponse.success([_provider_to_response(db, provider) for provider in providers])
    except Exception as e:
        logger.error(f"Error listing LLM providers: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to list LLM providers")
...
async def test_provider(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to test LLM provider")
...
async def discover_provider_models(
    ...
    try:
        ...
        return ShuResponse.success({"provider_id": provider_id, "discovered_models": models, "count": len(models)})
    except LLMProviderError as e:
...
async def create_model(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to create LLM model")
...
@router.get("/health")
async def health_check(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to check LLM health"
        ) from e
