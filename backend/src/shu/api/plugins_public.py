...
class PluginInfoResponse(BaseModel):
    ...
    op_auth: Optional[Dict[str, Any]] = None
...
async def list_plugins(
    ...
    for name, rec in (manifest or {}).items():
        try:
            ...
            req_ids = list(rec.required_identities or []) if getattr(rec, "required_identities", None) is not None else None
            ...
            allowed_feed_ops = list(rec.allowed_feed_ops or []) if getattr(rec, "allowed_feed_ops", None) is not None else None
    ...
    return ShuResponse.success(
        sorted(out, key=lambda x: (not x.enabled, x.display_name.casefold()))
    )
...
async def get_plugin(
    ...
    req_ids = list(rec.required_identities or []) if rec and getattr(rec, "required_identities", None) is not None else None
    ...
    allowed_feed_ops = list(rec.allowed_feed_ops or []) if rec and getattr(rec, "allowed_feed_ops", None) is not None else None
    display_name = (getattr(rec, "display_name", None) if rec else None) or (getattr(rec, "name", None) if rec else None) or name

    return ShuResponse.success(PluginInfoResponse(
        name=row.name,
...
async def execute_plugin(
    ...
    try:
        from ..services.plugin_identity import ensure_secrets_for_plugin, PluginIdentityError
        await ensure_secrets_for_plugin(plugin, name, str(user.id), body.params or {})
    except PluginIdentityError as pie:
        raise HTTPException(status_code=403, detail={
            "error": {"code": pie.code, "message": str(pie), "details": pie.details}
        })
    except HTTPException:
        raise
