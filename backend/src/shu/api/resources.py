...
from ..services.rag_processing_service import get_rag_service_stats, clear_rag_service_cache, cleanup_rag_services
...
async def check_resource_health(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to check resource health: {str(e)}"
        )
