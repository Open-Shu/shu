...
from ..schemas.rbac import (
    ...
    UserGroupMembershipCreate, UserGroupMembershipResponse,
    UserGroupListResponse, UserGroupMembershipListResponse
)
...
async def create_user_group(
    ...
    """
    logger.info("Creating user group", extra={
...
@router.get(
    ...
    description="List all user groups with pagination."
)
async def list_user_groups(
    ...
    """
    logger.info("Listing user groups", extra={
        "page": page,
        "page_size": page_size,
...
async def get_user_group(
    ...
    """
    logger.info("Getting user group", extra={
        "group_id": group_id,
        "requested_by": current_user.id
    })
    ...
    try:
        ...
        if "not found" in str(e).lower():
            raise HTTPException(
                ...
                detail=f"User group '{group_id}' not found"
            )
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to get user group"
        )
...
async def update_user_group(
    ...
    """
    logger.info("Updating user group", extra={
        "group_id": group_id,
...
async def delete_user_group(
    ...
    """
    logger.info("Deleting user group", extra={
        "group_id": group_id,
...
async def add_user_to_group(
    ...
    try:
        rbac_service = RBACService(db)
        membership = await rbac_service.add_user_to_group(
            group_id, membership_data, current_user.id
        )
...
async def remove_user_from_group(
    ...
    """
    logger.info("Removing user from group", extra={
        "group_id": group_id,
        "user_id": user_id,
    ...
    try:
        ...
        return ShuResponse.success(
            data={"removed_user_id": user_id, "group_id": group_id}
        )
...
async def list_group_members(
    ...
    Raises:
        HTTPException: If group not found
    """
    logger.info("Listing group members", extra={
        "group_id": group_id,
        "requested_by": current_user.id
    })
    ...
    try:
        ...
        for membership in memberships:
            membership_responses.append(UserGroupMembershipResponse(
                id=membership.id,
                user_id=membership.user_id,
                group_id=membership.group_id,
                role=membership.role,
        ...
        response_data = UserGroupMembershipListResponse(
            ...
            total_count=len(membership_responses)
        )
