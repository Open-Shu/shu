...
from ..models.rbac import KnowledgeBasePermission, UserGroupMembership, UserGroup
from ..models.knowledge_base import KnowledgeBase
from ..schemas.rbac import (
    KnowledgeBasePermissionResponse, KnowledgeBasePermissionListResponse,
    UserGroupMembershipResponse, UserGroupMembershipListResponse
)
...
async def get_current_user_kb_permissions(
    ...
    """
    logger.info("Getting current user KB permissions", extra={
...
async def get_current_user_group_memberships(
    ...
    """
    logger.info("Getting current user group memberships", extra={
        "user_id": current_user.id
    })
...
async def get_user_kb_permissions(
    ...
    if user_id != current_user.id and current_user.role != "admin":
        raise HTTPException(
            ...
            detail="Cannot view permissions for other users"
        )
    
    logger.info("Getting user KB permissions", extra={
        "user_id": user_id,
        ...
        for permission in all_permissions:
            ...
            if permission.group_id:
                group_result = await db.execute(
                    select(UserGroup.name).where(UserGroup.id == permission.group_id)
                )
                group_name = group_result.scalar_one_or_none()
            
            permission_responses.append(KnowledgeBasePermissionResponse(
                id=permission.id,
...
async def get_user_group_memberships(
    ...
    logger.info("Getting user group memberships", extra={
        "user_id": user_id,
        "requested_by": current_user.id
    })
    ...
    try:
        ...
        for membership in memberships:
            # Get group name
            group_result = await db.execute(
                select(UserGroup.name).where(UserGroup.id == membership.group_id)
            )
            group_name = group_result.scalar_one_or_none()
