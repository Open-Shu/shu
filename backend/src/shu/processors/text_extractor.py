...
from ..core.config import get_config_manager, ConfigurationManager
...
class TextExtractor:
    ...
    def __init__(self, config_manager: ConfigurationManager = None):
        ...
        self.supported_extensions = {'.pdf', '.docx', '.doc', '.rtf', '.xlsx', '.pptx', '.txt', '.md', '.html', '.htm', '.csv', '.py', '.js'}
    ...
    def register_ocr_thread(cls, job_id: str, thread: threading.Thread):
        ...
        with cls._thread_lock:
            ...
            logger.debug(f"Registered OCR thread for job {job_id}, total threads: {len(cls._active_ocr_threads[job_id])}")
        ...
        with cls._thread_lock:
            if job_id in cls._active_ocr_threads:
                ...
                if alive_threads:
                    ...
                    logger.info(f"Orphaned {len(alive_threads)} OCR threads for job {job_id} - they will complete naturally")
    ...
    async def extract_text(self, file_path: str, file_content: Optional[bytes] = None, use_ocr: bool = True, kb_config: Optional[Dict[str, Any]] = None, progress_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not file_ext and file_content:
            ...
            logger.debug("No file extension found, using fallback", extra={
                "file_path": file_path,
                "fallback_extension": file_ext
            })
        ...
        logger.debug("Using direct text extraction", extra={
            "file_path": file_path,
            "file_ext": file_ext,
            "use_ocr": use_ocr
        })
            ...
            if progress_context and progress_context.get('enhanced_tracker'):
                ...
                if tracker.current_document_tracker:
                    ...
                    if tracker.current_document_tracker.method != actual_method:
                        logger.info(f"Updating processing method from {tracker.current_document_tracker.method} to {actual_method}")
    ...
    async def _extract_text_direct(self, file_path: str, file_content: Optional[bytes] = None, progress_context: Optional[Dict[str, Any]] = None, use_ocr: bool = True) -> str:
        ...
        try:
            ...
            logger.debug("Successfully extracted and cleaned text", extra={
                "file_path": file_path,
            ...
            logger.error("Failed to extract text", extra={
                "file_path": file_path,
                "error": str(e)
            })
            raise
    ...
    def _create_progress_callback(self, progress_context: Dict[str, Any], use_ocr: bool = False):
        ...
        if not all([enhanced_tracker, sync_job_id, document_id]):
            logger.warning("Incomplete progress context provided", extra={
        ...
        try:
            ...
            logger.warning("No running event loop found when creating progress callback", extra={
        ...
        def progress_callback(current_page: int, total_pages: int, page_time: float = 0.0, page_text_length: int = 0):
            ...
            try:
                ...
                if not main_loop:
                    logger.warning("No event loop available for progress update", extra={
                        "sync_job_id": sync_job_id,
                        "document_id": document_id,
                ...
                def schedule_update():
                    ...
                    try:
                        ...
                        async def update_progress():
                            try:
                                if use_ocr:
                                    ...
                                    logger.debug("Updated OCR progress", extra={
                                        "sync_job_id": sync_job_id,
                                        "document_id": document_id,
                                        "current_page": current_page,
                                        "total_pages": total_pages,
                                    ...
                                    logger.debug("Updated fast extraction progress", extra={
                                        "sync_job_id": sync_job_id,
                                        "document_id": document_id,
                                        "current_page": current_page,
                                        "total_pages": total_pages,
                                ...
                                logger.warning(f"Progress update failed: {e}", extra={
                                    "sync_job_id": sync_job_id,
                                    "document_id": document_id,
                                    "current_page": current_page,
                                    "total_pages": total_pages,
                        ...
                        logger.error("Failed to schedule progress update", extra={
                            "sync_job_id": sync_job_id,
                            "document_id": document_id,
                            "error": str(e)
                        })
                ...
                logger.error("Failed to update progress in direct extraction", extra={
                    "sync_job_id": sync_job_id,
                    "document_id": document_id,
                    "current_page": current_page,
                    "total_pages": total_pages,
    ...
    async def _extract_text_pdf_with_progress(self, file_path: str, file_content: Optional[bytes] = None, progress_callback=None, use_ocr: bool = True, ocr_mode: str = "auto") -> str:
        """Extract text from PDF with page-by-page progress updates."""
        logger.debug("Extracting PDF text with progress updates", extra={"file_path": file_path, "use_ocr": use_ocr, "ocr_mode": ocr_mode})
        ...
        if ocr_mode == "fallback":
            ...
            try:
                ...
                if fast_text and fast_text.strip() and len(fast_text.strip()) > 50:  # Minimum threshold for meaningful text
                    logger.info("Fast extraction successful, skipping OCR", extra={
                        "file_path": file_path,
                        "text_length": len(fast_text.strip())
                    })
                    return fast_text
                else:
                    logger.info("Fast extraction yielded insufficient text, falling back to OCR", extra={
                        "file_path": file_path,
                ...
                logger.warning("Fast extraction failed, falling back to OCR", extra={
                    "file_path": file_path,
                    "error": str(e)
                })
                use_ocr = True
    ...
    async def _extract_pdf_text_only(self, file_path: str, file_content: Optional[bytes] = None, progress_callback=None) -> str:
        ...
        def _extract_text_only():
            ...
            try:
                ...
                for page_num in range(total_pages):
                    ...
                    logger.debug(f"Processed page {page_num + 1}/{total_pages}", extra={
                        "file_path": file_path,
    ...
    async def _extract_text_pdf_fast_only(self, file_path: str, file_content: Optional[bytes] = None) -> str:
            ...
            try:
                ...
                for page_num in range(total_pages):
                    ...
                    if self._current_sync_job_id and self.is_job_cancelled(self._current_sync_job_id):
                        logger.info(f"Fast extraction cancelled for job {self._current_sync_job_id}, stopping at page {page_num + 1}")
                        break
    ...
    async def _try_ocr_fallback(self, file_path: str, file_content: Optional[bytes] = None, progress_callback=None) -> str:
        ...
        if getattr(self.config_manager.settings, "ocr_execution_mode", "thread").lower() == "process":
            ...
            try:
                ...
                result = asyncio.run(self._extract_text_isolated(
                    file_path=file_path,
                ...
                if result and isinstance(result, dict) and result.get('text'):
                    ...
                    logger.info(f"Isolated OCR fallback successful with {extraction_method}", extra={
                        "file_path": file_path,
    ...
    async def _extract_pdf_ocr_direct(self, file_path: str, file_content: Optional[bytes] = None, progress_callback=None) -> str:
        ...
        try:
            ...
            total_pages = len(doc)
            logger.info(f"Starting direct OCR processing for {total_pages} pages", extra={"file_path": file_path})
            ...
            try:
                ...
                if text.strip():
                    ...
                    logger.info(f"OCR processing successful", extra={
                        "file_path": file_path,
                        "method": method,
                        "confidence": confidence,
                        "processing_time": processing_time,
            ...
            try:
                ...
                if text:
                    logger.info("PDF open failed; fell back to plain-text read for OCR test fixture", extra={"file_path": file_path})
                    return text
            except Exception as fallback_err:
                logger.warning(f"Plain-text fallback after OCR failure also failed: {fallback_err}", extra={"file_path": file_path})
            return ""
    ...
    async def _process_pdf_with_ocr_direct(self, doc, file_path: str, progress_callback=None):
        ...
        for page_num in range(total_pages):
            ...
            logger.info(f"Starting OCR processing for page {page_num + 1}/{total_pages}", extra={"file_path": file_path})
            ...
            while not ocr_complete.is_set():
                ...
                if time.time() - start_time > max_wait_time:
                    logger.error(f"OCR timeout on page {page_num + 1} after {max_wait_time} seconds", extra={"file_path": file_path})
                    # Force completion and use empty result
                    ocr_complete.set()
                    ocr_result = []
                    break
                ...
                logger.debug(f"OCR processing page {page_num + 1}, progress update {progress_counter}", extra={"file_path": file_path})
            ...
            try:
                ...
                logger.error(f"OCR thread join timeout on page {page_num + 1}", extra={"file_path": file_path})
            ...
            try:
                ...
                logger.info(f"Completed OCR processing for page {page_num + 1}/{total_pages}", extra={"file_path": file_path})

            except Exception as e:
                logger.warning(f"EasyOCR processing failed on page {page_num + 1}: {e}", extra={"file_path": file_path})
    ...
    def _process_pdf_with_tesseract_direct(self, doc, file_path: str, progress_callback=None):
        ...
        for page_num in range(total_pages):
            # Check for job cancellation before processing each page
            if self._current_sync_job_id and self.is_job_cancelled(self._current_sync_job_id):
                logger.info(f"Tesseract OCR cancelled for job {self._current_sync_job_id}, stopping at page {page_num + 1}")
                break
    ...
    def _extract_text_pdf_fallback(self, file_path: str, file_content: Optional[bytes] = None) -> str:
    ...
    async def _extract_text_plain(self, file_path: str, file_content: Optional[bytes] = None) -> str:
    ...
    async def _extract_text_pdf(self, file_path: str, file_content: Optional[bytes] = None) -> str:
        ...
        def _try_ocr():
            ...
            try:
                ...
                for page_num in range(page_count):
                    # Check for job cancellation before processing each page
                    if self._current_sync_job_id and self.is_job_cancelled(self._current_sync_job_id):
                        logger.info(f"Legacy OCR cancelled for job {self._current_sync_job_id}, stopping at page {page_num + 1}")
                        break
                    ...
                    try:
                        ...
                        logger.debug(f"Converted page {page_num + 1} to image: {img.size}", extra={"file_path": file_path})

                        # Extract text using OCR
                        page_text = pytesseract.image_to_string(img)
                        if page_text.strip():
                            text += page_text + "\n"
                            logger.debug(f"OCR extracted {len(page_text.strip())} characters from page {page_num + 1}", extra={"file_path": file_path})
                        else:
                            logger.debug(f"OCR found no text on page {page_num + 1}", extra={"file_path": file_path})

                    except Exception as page_error:
                        logger.warning(f"OCR failed on page {page_num + 1}: {page_error}", extra={"file_path": file_path})
                        continue
                ...
                if final_text:
                    logger.debug(f"OCR completed successfully, extracted {len(final_text)} total characters", extra={"file_path": file_path})
                else:
                    logger.warning(f"OCR completed but extracted no text from any pages", extra={"file_path": file_path})
        ...
        for method_name, method_func in text_methods:
            try:
                ...
                if result:
                    logger.info(f"Successfully extracted text using {method_name} ({len(result)} characters)", extra={"file_path": file_path})
                    return result
                else:
                ...
                extraction_errors.append(f"{method_name}: {str(e)}")
                continue
    ...
    async def _extract_text_docx(self, file_path: str, file_content: Optional[bytes] = None) -> str:
        ...
        def _try_textract():
            ...
            try:
                ...
                logger.debug(f"textract DOCX extraction failed: {str(e)}")
                return None
        
        # Try multiple DOCX extraction methods in order of preference
        extraction_methods = [
            ("python-docx", _try_python_docx),
            ("textract", _try_textract)
        ]
        ...
        for method_name, extractor_func in extraction_methods:
            try:
                ...
                if text and text.strip():
                    logger.debug(f"Successfully extracted DOCX text using {method_name}", extra={
                        "file_path": file_path,
                        "method": method_name,
                ...
                logger.debug(f"{method_name} extraction failed: {str(e)}")
    ...
    async def _extract_text_doc(self, file_path: str, file_content: Optional[bytes] = None) -> str:
    ...
    async def _extract_text_rtf(self, file_path: str, file_content: Optional[bytes] = None) -> str:
    ...
    async def _extract_text_html(self, file_path: str, file_content: Optional[bytes] = None) -> str:
    ...
    async def _extract_text_fallback(self, file_path: str, file_content: Optional[bytes] = None) -> str:
        ...
        try:
            def _read_file():
                if file_content:
                    ...
                    file_ext = Path(file_path).suffix.lower()
                    binary_extensions = {
                        '.pdf', '.docx', '.doc', '.pptx', '.xlsx', '.zip', '.exe', '.dll', 
                        '.so', '.dylib', '.bin', '.dat', '.obj', '.class', '.jar', '.war',
                        '.ear', '.apk', '.ipa', '.dmg', '.iso', '.img', '.vhd', '.vmdk'
                    }
            ...
            logger.error("Fallback extraction failed", extra={
                "file_path": file_path,
                "error": str(e)
            })
            return f"[Error: Could not extract text from {file_path} - {str(e)}]"

    async def _extract_text_email(self, file_path: str, file_content: Optional[bytes] = None) -> str:
        """Extract text from email content (Gmail messages, etc.)."""
        try:
            if file_content:
                # Email content is already in text format (from Gmail API)
                text = file_content.decode('utf-8')
                logger.debug("Successfully extracted email text from content", extra={
                    "file_path": file_path,
            ...
            logger.error("Failed to extract email text", extra={
                "file_path": file_path,
                "error": str(e)
            })
            return f"[Error: Could not extract email text from {file_path} - {str(e)}]"
