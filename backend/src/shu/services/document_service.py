...
from ..schemas.document import (
    ...
    DocumentList, DocumentSummary, DocumentChunkResponse, DocumentStats,
    DocumentSearchRequest, DocumentSearchResponse, ProcessingStatus
)
from ..core.exceptions import KnowledgeBaseNotFoundError, DocumentNotFoundError, ValidationError
...
class DocumentService:
    ...
    async def create_document(self, doc_data: DocumentCreate) -> DocumentResponse:
        ...
        logger.debug("Creating document", extra={
        ...
        if existing:
            ...
            logger.warning("Document already exists, returning existing", extra={
                "doc_id": existing.id,
        ...
        logger.debug("Created document", extra={
            "doc_id": document.id,
    ...
    async def list_documents(
        ...
        logger.debug("Listing documents", extra={
        ...
        logger.debug("Listed documents", extra={
    ...
    async def get_documents_by_source_type(self, source_type: str) -> List[DocumentResponse]:
        ...
        result = await self.db.execute(
            select(Document).where(Document.source_type == source_type)
        )
        documents = result.scalars().all()
        
        result_list = [DocumentResponse.from_orm(doc) for doc in documents]
        
        logger.debug("Retrieved documents by source type", extra={
            "source_type": source_type,
    ...
    async def get_document_by_source_id(self, knowledge_base_id: str, source_id: str) -> Optional[Document]:
        """Get a document by its source ID within a knowledge base."""
        logger.debug("Getting document by source ID", extra={
            "kb_id": knowledge_base_id,
            "source_id": source_id
        })
        ...
        if document:
            logger.debug("Found document by source ID", extra={
                "doc_id": document.id,
                "title": document.title
            })
        else:
            logger.debug("No document found for source ID", extra={
                "kb_id": knowledge_base_id,
                "source_id": source_id
            })
    ...
    async def get_document_stats(self, knowledge_base_id: Optional[str] = None) -> DocumentStats:
        ...
        total_words_result = await self.db.execute(
            ...
            .select_from(query.subquery())
            .where(Document.word_count.isnot(None))
        )
        ...
        # File type breakdown
        file_type_query = query.with_entities(
            Document.file_type,
        ...
        file_type_breakdown = {
            result.file_type: result.count for result in file_type_results
        }
        
        # Source type breakdown
        source_type_query = query.with_entities(
        ...
        source_type_breakdown = {
            result.source_type: result.count for result in source_type_results
        }
        ...
        processing_status_breakdown = {
            result.processing_status: result.count for result in status_results
        }
        ...
        logger.debug("Retrieved document statistics", extra={
    ...
    async def mark_document_processed(self, doc_id: str, word_count: int, character_count: int, chunk_count: int) -> None:
        """Mark a document as processed with stats."""
        logger.debug("Marking document as processed", extra={
            "doc_id": doc_id,
            "word_count": word_count,
            "character_count": character_count,
            "chunk_count": chunk_count
    ...
    async def mark_document_error(self, doc_id: str, error_message: str) -> None:
        ...
        logger.debug("Marking document as error", extra={
            "doc_id": doc_id,
    ...
    async def get_document_chunks(self, doc_id: str) -> List[DocumentChunkResponse]:
        ...
        logger.debug("Retrieved document chunks", extra={
            "doc_id": doc_id,
    ...
    async def search_documents(self, search_request: DocumentSearchRequest) -> DocumentSearchResponse:
        ...
        logger.debug("Searching documents", extra={
        ...
        logger.debug("Searched documents", extra={
            "query": search_request.query,
    ...
    async def get_documents_by_knowledge_base(self, knowledge_base_id: str) -> List[DocumentResponse]:
        ...
        logger.debug("Retrieved documents by knowledge base", extra={
            "kb_id": knowledge_base_id,
            "count": len(result_list)
        })
    ...
    async def get_pending_documents(self, knowledge_base_id: Optional[str] = None) -> List[DocumentResponse]:
        ...
        logger.debug("Retrieved pending documents", extra={
            "kb_id": knowledge_base_id,
            "count": len(result_list)
        })
