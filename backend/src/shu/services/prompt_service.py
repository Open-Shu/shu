...
from ..models.prompt import Prompt, PromptAssignment, EntityType
...
from ..schemas.prompt import (
    ...
    PromptAssignmentCreate, PromptAssignmentResponse,
    PromptQueryParams, PromptUsageStats, PromptSystemStats
)
from ..core.exceptions import (
    ShuException, NotFoundError, ValidationError, ConflictError
)
from ..utils.prompt_utils import (
    has_citation_instructions, get_effective_reference_setting, get_citation_conflict_info
)
...
class PromptService:
    ...
    async def create_prompt(self, prompt_data: PromptCreate) -> PromptResponse:
        ...
        existing = await self._get_prompt_by_name_and_type(
            prompt_data.name, prompt_data.entity_type.value
        )
        if existing:
    ...
    async def get_entity_prompts(self, entity_id: str, entity_type: str, active_only: bool = True) -> List[PromptResponse]:
        """
        ...
        stmt = (
            ...
            .where(
                and_(
                    ...
                    PromptAssignment.entity_id == entity_id
                )
            )
        )
    ...
    async def get_system_stats(self) -> PromptSystemStats:
        ...
        # Active prompts
        active_prompts_result = await self.db.execute(
        ...
        prompts_by_type_result = await self.db.execute(
            select(Prompt.entity_type, func.count(Prompt.id))
    ...
    async def _get_prompt_by_id(self, prompt_id: str) -> Optional[Prompt]:
        """Get prompt by ID."""
        stmt = select(Prompt).where(Prompt.id == prompt_id)
        result = await self.db.execute(stmt)
        return result.scalar_one_or_none()
    
    async def _get_prompt_by_name_and_type(self, name: str, entity_type: str) -> Optional[Prompt]:
        """Get prompt by name and entity type."""
        stmt = select(Prompt).where(
            and_(Prompt.name == name, Prompt.entity_type == entity_type)
        )
        result = await self.db.execute(stmt)
        return result.scalar_one_or_none()
    
    async def _get_assignment(self, prompt_id: str, entity_id: str) -> Optional[PromptAssignment]:
    ...
    async def get_kb_prompts_with_reference_info(
        ...
        for prompt in prompts:
            ...
            effective_references, reason = get_effective_reference_setting(
                kb_include_references, prompt.content
            )
    ...
    def analyze_citation_handling(
        ...
        effective_references, reason = get_effective_reference_setting(
            kb_include_references, prompt_content
        )
    ...
    async def get_model_config_kb_prompts(self, model_config_id: str) -> Dict[str, PromptResponse]:
        ...
        try:
            ...
            raise ShuException(f"Failed to get model config KB prompts: {str(e)}", "GET_MODEL_CONFIG_KB_PROMPTS_ERROR")
    ...
    async def assign_prompt_to_model_config_kb(
        ...
        try:
            ...
            raise ShuException(f"Failed to assign prompt to model config KB: {str(e)}", "ASSIGN_MODEL_CONFIG_KB_PROMPT_ERROR")

    async def remove_prompt_from_model_config_kb(
        self,
        model_config_id: str,
        knowledge_base_id: str
    ) -> bool:
        ...
        try:
            ...
            raise ShuException(f"Failed to remove prompt from model config KB: {str(e)}", "REMOVE_MODEL_CONFIG_KB_PROMPT_ERROR")
