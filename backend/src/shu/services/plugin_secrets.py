...
async def list_secret_keys(
    ...
    *,
    ...
    """
    return await storage_list_keys(
        name, SECRET_NAMESPACE, scope=normalize_scope(scope), user_id=user_id
    )
...
async def list_secrets_meta(
    ...
    *,
    ...
    return await storage_list_meta(
        name, SECRET_NAMESPACE, scope=normalize_scope(scope), user_id=user_id
    )
...
async def get_secret(
    ...
    *,
    ...
    """
    raw = await storage_get_scoped(
        name, SECRET_NAMESPACE, key, scope=normalize_scope(scope), user_id=user_id
    )
    if not raw:
        return None
    enc = raw.get("v")
